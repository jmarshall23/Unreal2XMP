	...
	while( ParseLine(&Buffer,StrLine) )
	{
		...
		else if( GetBEGIN(&Str,TEXT("ACTOR")) )
		{
			...
			if( ParseObject<UClass>( Str, TEXT("CLASS="), TempClass, ANY_PACKAGE ) )
			{
				...
				AActor* Actor = NULL;
				if( Found && Found->IsA( TEXT("ParticleGenerator") ) )
				{
					UBOOL GIsSavedScriptableSaved = 1;
					Exchange(GIsScriptable,GIsSavedScriptableSaved);
					struct FuncParms{ class AActor* A; class ULevel* L; };
					FuncParms Parms;
					Parms.A = NULL;
					Parms.L = InLevel;
					Found->UObject::ProcessEvent(Found->FindFunctionChecked(FName(TEXT("Duplicate"),FNAME_Intrinsic)),&Parms);
					Actor = Parms.A;
					Exchange(GIsScriptable,GIsSavedScriptableSaved);
					Actor->StaticFilterState=FS_Maybe;
					Actor->ActorRenderData=NULL;
					Actor->LightRenderData=NULL;
					Actor->RenderRevision=0;
				}
				else Actor = InLevel->SpawnActor( TempClass, ActorName, FVector(0,0,0), FRotator(0,0,0), NULL, 1, 0 );
				check(Actor);
				Actor->bTempEditor = 0;
#else
...
)
{
	...
	// BMP
	//
	if( (Length>=sizeof(FBitmapFileHeader)+sizeof(FBitmapInfoHeader)) && Buffer[0]=='B' && Buffer[1]=='M' && appStricmp(Type,TEXT("bmp"))==0 )
    {
	...
	// PCX
	//
	else if( Length >= sizeof(FPCXFileHeader) && PCX->Manufacturer==10 && appStricmp(Type, TEXT("pcx"))==0 )
	{
	...
	// TGA
	//
	else if( Length >= sizeof(FTGAFileHeader) && appStricmp(Type, TEXT("tga"))==0 )
	{
	...
	// DDS
	//
	else if( Length >= sizeof(FDDSFileHeader) && DDS->Magic==0x20534444 && appStricmp(Type, TEXT("dds"))==0 )
	{
