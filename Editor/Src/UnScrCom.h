...
public:
	...
	UBOOL IsRange() const
	{
		return Type==CPT_Struct && appStricmp(*Struct->GetFName(),TEXT("Range"))==0;
	}
	...
	UBOOL MatchesType( const FPropertyBase& Other, UBOOL Identity ) const
	{
		...
		if( PropertyFlags & CPF_OutParm )
		{
            ...
            if( Type!=CPT_Struct ) //  This allows for derived structs to be passed by reference.
				Identity = 1;
		}
...
public:
	...
	union
	{
		...
		TCHAR	RangeBytes[sizeof(FRange)];			 // If CPT_Struct && IsRange().
	...
	void SetConstRange( FRange &InRange )
	{
		(FPropertyBase&)*this   = FPropertyBase(CPT_Struct);
		Struct                  = FindObjectChecked<UStruct>( ANY_PACKAGE, TEXT("Range") );//!!
		*(FRange *)RangeBytes   = InRange;
		TokenType		        = TOKEN_Const;
	}
	...
	UBOOL GetConstByte( BYTE& B )
	{
		...
		else if( TokenType==TOKEN_Const && Type==CPT_Float && Float>=0 && Float<256 && Float==(INT)Float)
		{
			B = Float;
			return 1;
		}
	...
	UBOOL GetConstRange( FRange& r )
	{
		if( TokenType==TOKEN_Const && IsRange() )
		{
			r = *(FRange *)RangeBytes;
			return 1;
		}
...
public:
	...
	UBOOL			UseLineNumbers;			// Dump line number instructions to code stream.
	UBOOL			InDefaultPropContext;	// GetContext() is in default properties mode
	UBOOL			bEmitDebugInfo;			// Emit debug information to the stream.
	...
	void			EmitLineNumber(); // CDH line numbers
	INT				EmitOuterContext();
	void			EmitDebugInfo( TCHAR* AdditionalInfo );
