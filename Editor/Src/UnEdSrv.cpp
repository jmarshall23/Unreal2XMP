...
ESConstant GetConstant(FString ConstantName)
{
	static FString ShaderConstantNames[] =
	{
		TEXT(""),
		TEXT(""),
		TEXT("view_proj_matrix"),
		TEXT("world_view_proj_matrix"),
		TEXT("world_matrix"),
		TEXT("inv_view_matrix"),
		TEXT("view_matix"),
		TEXT("inv_world_matrix"),
		TEXT("time"),
		TEXT("cos(time)"),
		TEXT("sin(time)"),
		TEXT("tan(time)"),
		TEXT("eye"),
		TEXT("xy_circle"),
		TEXT("nearest_light_pos_1"),
		TEXT("inv_light_radius_1"),
		TEXT("nearest_light_pos_2"),
		TEXT("inv_light_radius_2"),
		TEXT("light_color_1"),
		TEXT("light_color_2"),
		TEXT("ambient_light_color")
	};

	for(INT TypeCounter = 0; TypeCounter < EVC_Max; TypeCounter++)
	{
		if(ShaderConstantNames[TypeCounter] == ConstantName)
		{
			return (ESConstant)TypeCounter;
		}
	}
	return EVC_MaterialDefined;
}



UBOOL UEditorEngine::Exec_Shader( const TCHAR* Str, FOutputDevice& Ar )
{
	FString File;
	Parse(Str,TEXT("FILE="),File);

	FShaderLoader* ShaderLoader = new FShaderLoader(File);
	...
	for(INT ShaderIndex = 0; ShaderIndex < ShaderLoader->NumShaders; ShaderIndex++)
	{
		FName Name(*(ShaderLoader->Shaders[ShaderIndex].Name));
	
		UHardwareShader* NewShader = Cast<UHardwareShader>(StaticFindObject( UHardwareShader::StaticClass(), Pkg, *Name ));

		// Create new shader object.
		if(!NewShader)
			NewShader = Cast<UHardwareShader>(StaticConstructObject( UHardwareShader::StaticClass(), Pkg, Name, RF_Public|RF_Standalone ));

		// Take all the data from ShaderLoader and fill in NewShader
 
		// ==== Load Mapping ====
		NewShader->StreamMapping.Empty();

		// For each stream mapping in the loader
		for(INT RegIndex = 0; RegIndex < 16; RegIndex++)
		{
			// find the next register
			for(INT MapIndex = 0; MapIndex < ShaderLoader->Shaders[ShaderIndex].Passes[0].MyMapping->NumRegMappings; MapIndex++)
			{
				// Only use the mapping from the first pass, we do not yet support per-pass mappings
				if(ShaderLoader->Shaders[ShaderIndex].Passes[0].MyMapping->RegMappings[MapIndex].Register == RegIndex)
				{
					// Found the mapping, now add it.
					switch(ShaderLoader->Shaders[ShaderIndex].Passes[0].MyMapping->RegMappings[MapIndex].Usage)
					{
					case 0:
						NewShader->StreamMapping.AddItem(FVF_Position);
						break;
					case 3:
						NewShader->StreamMapping.AddItem(FVF_Normal);
						break;
					case 5:
						NewShader->StreamMapping.AddItem(FVF_Diffuse);
						break;
					case 6:
						NewShader->StreamMapping.AddItem(FVF_Specular);
						break;
					case 7:
						NewShader->StreamMapping.AddItem(FVF_TexCoord0);
						break;
					case 8:
						NewShader->StreamMapping.AddItem(FVF_Tangent);
						break;
					case 9:
						NewShader->StreamMapping.AddItem(FVF_Binormal);
						break;
					default:
						check(0); // unknown usage
					}
				}
			}
		}
		...
		for(INT PassIndex = 0; PassIndex < ShaderLoader->Shaders[ShaderIndex].NumPasses; PassIndex++)
		{
			NewShader->VertexShaderText[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderText;
		}

		// == Load Vertex Shader Constants ==
		for(INT PassIndex = 0; PassIndex < ShaderLoader->Shaders[ShaderIndex].NumPasses; PassIndex++)
		{
			for(INT ConstIndex = 0; ConstIndex < ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].NumVSConstantMappings; ConstIndex++)
			{
				ESConstant Constant = GetConstant(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Name);

				if(Constant != EVC_MaterialDefined)
				{
					NewShader->VSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].Register][PassIndex].Type = Constant;
				}
				// Hard-coded constants
				else
				{
					NewShader->VSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].Register][PassIndex].Type = EVC_MaterialDefined;
					
					if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::VECTOR)
					{
						appMemcpy(&(NewShader->VSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].Register][PassIndex].Value),
							ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
					}
					else if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::MATRIX)
					{
						for(INT i = 0; i < 4; i++)
						{
							appMemcpy(&(NewShader->VSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex + i].Register][PassIndex].Value),
								&(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Values), sizeof(FLOAT) * 4);
						}
					}
					else if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::FLOATX)
					{
						appMemcpy(&(NewShader->VSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].Register][PassIndex].Value), 
							ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
					}
					else if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::INTX)
					{
						appMemcpy(&(NewShader->VSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].Register][PassIndex].Value), 
							ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].VertexShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
					}
		...
		for(INT PassIndex = 0; PassIndex < ShaderLoader->Shaders[ShaderIndex].NumPasses; PassIndex++)
		{
			for(INT ConstIndex = 0; ConstIndex < ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].NumPSConstantMappings; ConstIndex++)
			{

				ESConstant Constant = GetConstant(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Name);

				if(Constant != EVC_MaterialDefined)
				{
					NewShader->PSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].Register][PassIndex].Type = Constant;
				}
				// Hard-coded constants
				else
				{
					NewShader->PSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].Register][PassIndex].Type = EVC_MaterialDefined;
					
					if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::VECTOR)
					{
						appMemcpy(&(NewShader->PSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].Register][PassIndex].Value),
							ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
					}
					else if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::MATRIX)
					{
						for(INT i = 0; i < 4; i++)
						{
							appMemcpy(&(NewShader->PSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex + i].Register][PassIndex].Value),
								ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
						}
					}
					else if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::FLOATX)
					{
						appMemcpy(&(NewShader->PSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].Register][PassIndex].Value), 
							ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
					}
					else if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Type == FShaderLoader::Constant::INTX)
					{
						appMemcpy(&(NewShader->PSConstants[ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].Register][PassIndex].Value), 
							ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderConstantMappings[ConstIndex].MyConstant->Values, sizeof(FLOAT) * 4);
					}
		...
		// === Load Pixel Shader Text ===
		for(INT PassIndex = 0; PassIndex < ShaderLoader->Shaders[ShaderIndex].NumPasses; PassIndex++)
		{
			NewShader->PixelShaderText[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].PixelShaderText;
		}

		// === Load Textures ===
		for(INT PassIndex = 0; PassIndex < ShaderLoader->Shaders[ShaderIndex].NumPasses; PassIndex++)
		{
			for(INT TextureIndex = 0; TextureIndex < MAX_TEXTURE_STAGES; TextureIndex++)
			{
				if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex])
				{
					FString TextureName = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex]->Filename;

					// grab only the filename. The part after the "\" not including the extension
					TextureName = TextureName.Right(TextureName.Len() - TextureName.InStr(TEXT("\\"), 1) - 1);
					TextureName = TextureName.Left(TextureName.Len() - 4);

					UTexture* New = Cast<UTexture>(StaticFindObject( UTexture::StaticClass(), ANY_PACKAGE, *TextureName));
					if(New)
						NewShader->Textures[TextureIndex + (PassIndex * MAX_TEXTURE_STAGES)] = New;
					else if(TextureName != TEXT("Diffuse"))
						GWarn->Logf(NAME_Warning, TEXT("Unable to find texture %s in for new shader"), *TextureName);
				}
			}
		}

		// === Load Render State Info ===
		for(INT PassIndex = 0; PassIndex < ShaderLoader->Shaders[ShaderIndex].NumPasses; PassIndex++)
		{	
			NewShader->AlphaBlending[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].AlphaBlending;
			NewShader->AlphaTest[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].AlphaTest;
			NewShader->AlphaRef[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].AlphaRef;
			NewShader->ZTest[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].ZTest;
			NewShader->ZWrite[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].ZWrite;
			NewShader->SrcBlend[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].SrcBlend;
			NewShader->DestBlend[PassIndex] = ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].DestBlend;
		}

		NewShader->NumPasses = ShaderLoader->Shaders[ShaderIndex].NumPasses;

		NewShader->Changed();

		// Done loading the shader

		
		// === Handle batch-shader creation ===
		FName PackageName;
		if(Parse(Str, TEXT("BATCH_PACKAGE="), PackageName) && PackageName != NAME_None)
		{
	
			UPackage* BatchPackage = CreatePackage(NULL,*PackageName);
			BatchPackage->bDirty = 1;
	
			//The nonspecial texture names in the shader (e.g. "Diffuse" and "Bump" but not "Normalizer")
			// 
			// - Find the first tex name
			// - Determine the group that corresponds to this tex name
			// - Iterate through all the elements of this group
			//   - Start a copy
			//   - Fill in the texture ref for the first texture
			//   - For each other tex name, do a lookup using the current texture's name and the shader's texture ref name
			//   - if all found, commit the copy.

			int FirstTexPass = -1;
			int FirstTexTexture = -1;
			for(INT PassIndex = 0; PassIndex < NewShader->NumPasses; PassIndex++)
			{
				for(INT TextureIndex = 0; TextureIndex < MAX_TEXTURE_STAGES; TextureIndex++)
				{
					if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex] && 
						!(NewShader->Textures[TextureIndex + (PassIndex * MAX_TEXTURE_STAGES)])) 
					{
						FirstTexPass = PassIndex;
						FirstTexTexture = TextureIndex;
						break;
					}
				}
			}
			if (FirstTexPass == -1 && FirstTexTexture == -1)
			{
				GWarn->Logf(TEXT("Batch Import of shader requested, but can't find any free texture variables in the shader."));
			}
			else 
			{

				FString GroupName = ShaderLoader->Shaders[ShaderIndex].Passes[FirstTexPass].Textures[FirstTexTexture]->Name;
				UPackage* GenGroup = CreatePackage(BatchPackage,*GroupName);
				// iterate over all the textures in this group
				for(TObjectIterator<UTexture> TextureIt; TextureIt; ++TextureIt)
				{
					if (TextureIt->GetOuter() == GenGroup) 
					{
						FString Name = FString::Printf(TEXT("%s_%s"), TextureIt->GetName(), ShaderLoader->Shaders[ShaderIndex].Name);
						UHardwareShader* BatchedShader =	CastChecked<UHardwareShader>(UHardwareShader::StaticConstructObject( NewShader->GetClass(), Pkg, *Name, RF_Public|RF_Standalone, NewShader, GError ));
						*BatchedShader = *NewShader;
						
							
						// TODO Al: Temp hack to make things work while template issues are resolved. Might result in memory leak or other terribleness
						BatchedShader->InitDuplicate();
						BatchedShader->PostLoad();
						BatchedShader->PostEditChange();
						...
						for(INT PassIndex = 0; PassIndex < NewShader->NumPasses; PassIndex++)
						{
							...
							{
								if(ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex] && 
									!(BatchedShader->Textures[TextureIndex + (PassIndex * MAX_TEXTURE_STAGES)]))
								{

									UPackage* TexGroup = CreatePackage(BatchPackage,*ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex]->Name);
									UTexture* Tex = Cast<UTexture>(StaticFindObject(UTexture::StaticClass(),TexGroup,TextureIt->GetName(),1));

									if(Tex)
									{
										BatchedShader->Textures[TextureIndex + (PassIndex * MAX_TEXTURE_STAGES)] = Tex;
			
										// If this texture is called diffuse, use it as the fallback material
										if( ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex]->Name == TEXT("Diffuse") || 
											ShaderLoader->Shaders[ShaderIndex].Passes[PassIndex].Textures[TextureIndex]->Name == TEXT("diffuse") )
										{
											BatchedShader->FallbackMaterial = BatchedShader->Textures[TextureIndex + (PassIndex * MAX_TEXTURE_STAGES)];
										}
									} 
									else
									{
										GWarn->Logf(TEXT("Batch Import failure: Couldn't find texture %s->%s->%s."));
									}
								}
							}
						}
...
UBOOL UEditorEngine::SafeExec( const TCHAR* InStr, FOutputDevice& Ar )
{
	...
	else if(ParseCommand(&Str,TEXT("SHADER")))
	{
	    if (Exec_Shader(Str, Ar))
			return 1;
	}
		...
		if( ParseCommand(&Str,TEXT("Import")) )
		{
			...
			if( PkgName!=NAME_None && Parse( Str, TEXT("File="), TempFname, ARRAY_COUNT(TempFname) ) )
			{
				...
				if( Texture )
				{
					...
					Ar.Logf( NAME_Error, 
						TEXT("Import texture %s from %s failed"), TempName, TempFname ); 
				GWarn->EndSlowTask();
				GCache.Flush( 0, ~0, 1 );
			}
...
UBOOL UEditorEngine::Exec_StaticMesh( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("ADD")) )		// STATICMESH ADD NAME=<name> [ SNAP=<0/1> ]
	{
		...
		if( ParseObject<UStaticMesh>(Str,TEXT("NAME="),StaticMesh,ANY_PACKAGE) )
		{
			...
			// Don't batch meshes with hardware shaders on them.
			for(INT SectionIndex = 0;SectionIndex < StaticMesh->Sections.Num();SectionIndex++)
			{
				UMaterial*	Material = StaticMesh->GetSkin(Actor,SectionIndex);
				if(Material->IsA(UHardwareShader::StaticClass()))
				{
					Actor->bDontBatch=true;
					break;
				}
			}
		...
		for( INT i=0; i<Level->Actors.Num(); i++ )
		{
			...
			if(Actor)
			{
				for(INT ProjectorIndex = 0;ProjectorIndex < Actor->Projectors.Num();ProjectorIndex++)
					Actor->Projectors(ProjectorIndex)->RemoveReference();
				for(INT ProjectorIndex = 0;ProjectorIndex < Actor->StaticMeshProjectors.Num();ProjectorIndex++)
					Actor->StaticMeshProjectors(ProjectorIndex)->RenderInfo->RemoveReference();
				Actor->Projectors.Empty();
				Actor->StaticMeshProjectors.Empty();
			}
		}
		for( INT i=0; i<Level->Actors.Num(); i++ )
		{
...
UBOOL UEditorEngine::Exec_Obj( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("SavePackage")) )
	{
		...
		&&	ParseObject<UPackage>( Str, TEXT("Package="), Pkg, NULL ) )
		{
			...
			if( DiskSize > -1 && DiskSize - NewSize > 0 )	// Use a buffer of 100 bytes to offset stupid differences that sometimes come up
			{
...
UBOOL UEditorEngine::Exec_Audio( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	if( ParseCommand(&Str,TEXT("PLAY")) )
	{
		...
		if( Audio )
		{
			...
			Ar.Logf( TEXT("Can't find viewport for sound") );
		return 1;
	}
...
UBOOL UEditorEngine::Exec_Anim( const TCHAR* Str, FOutputDevice& Ar )
{
	...
	// Hack to comform (for now, only does a check) the bone hierarchy of one mesh to another.
	...
	else if( ParseCommand( &Str, TEXT("IMPORT")) )
	{
		...
		{	
			...
			if( appStrstr(TempFname,TEXT(".PSK") ) )
			{
				...
				// TODO -> change to use command line's package _and_ group, just like textures.
				FName PkgName; // = ParentContext ? ParentContext->GetFName() : NAME_None;
...
UBOOL UEditorEngine::Exec( const TCHAR* Stream, FOutputDevice& Ar )
{
	...
	else if( ParseCommand(&Str,TEXT("SETCURRENTPARTICLESYSTEM")) )
	{
		ParseObject<AActor>( Str, TEXT("IMAGE="), CurrentParticleSystem, ANY_PACKAGE );
		Ar.Logf( TEXT("CurrentParticleSystem=%s"), CurrentParticleSystem->GetName() );
		return 1;
	}
...
void UEditorEngine::edactCreateNewParticleSystem( ULevel* Level, AActor* Image )
{
	...
	if( Image->bDeleteMe )
	{
		debugf( NAME_Warning, TEXT("CreateNewParticleSystem failed : Image.bDeleteMe == true") );
		return;
	}
	...
	AActor* Actor = Level->SpawnActor( Image->GetClass(), NAME_None, Location, Image->Rotation, Image );
	if( Actor )
	{
		Actor->bSelected = 1;
		...
		debugf( NAME_Log, TEXT("Added %s successfully"), Actor->GetClass()->GetName() );
	}
	else GWarn->Logf( TEXT("Unable to spawn particle at %s!"), *Location.String() );
	Trans->End();
