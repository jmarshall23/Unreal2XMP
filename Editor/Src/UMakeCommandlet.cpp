...
class UMakeCommandlet : public UCommandlet
{
	...
	INT Main( const TCHAR* Parms )
	{
...
#if 1 //NEW (mdf)
		UBOOL bDebug = ParseParam(appCmdLine(), TEXT("debug"));
		UBOOL bNoLineInfo = ParseParam(appCmdLine(), TEXT("nolineinfo"));
		
		if( bDebug || bNoLineInfo )
		{
			GWarn->Logf( NAME_Log, TEXT(""));
			if( bDebug )
				GWarn->Logf( NAME_Log, TEXT("debug build"));
			if( bNoLineInfo )
				GWarn->Logf( NAME_Log, TEXT("not generating line numbers"));
			GWarn->Logf( NAME_Log, TEXT(""));
		}
#endif
		for( INT i=0; i<GEditor->EditPackages.Num(); i++ )
		{
			...
			GWarn->Log( NAME_Heading, FString::Printf(TEXT("%s - %s"),Pkg,ParseParam(appCmdLine(), TEXT("DEBUG"))? TEXT("Debug") : TEXT("Release")));
...
#if 1 //NEW (mdf) UCC make complete sound
		if( ParseParam(appCmdLine(),TEXT("donesound")) )
			appSound();
#endif
		GIsRequestingExit=1;
		return 0;
