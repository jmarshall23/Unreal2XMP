...
static INT Compare(FStaticMeshTriangle& A,FStaticMeshTriangle& B)
{
				...
				while( ParseLine( &Buffer, StrLine ) )
				{
					...
					else if ( Group == GROUP_GEOM )
					{
						...
						if( StrLine == TEXT("}") )
						{
							...
							BaseTVertexIndex = MappingChannels(CurrentMappingChannel).TexCoord.Num();
						}
							...
							else
							{
									...
									else
									{
										...
										else
										{
											...
											FString	SmoothingString;
												...
												// Cut off smoothing group(s) part - until EOL or next tag.												
												SmoothingString = StrLine.Right( StrLine.Len() - SmGroupsLocation - SmoothTag.Len() );
												INT NextTagLocation = SmoothingString.InStr(TEXT("*"));
												if( NextTagLocation != -1 )
													SmoothingString = SmoothingString.Left( NextTagLocation );

												while(SmoothingString.Len())
												{
													// Process comma-separated multiple smoothing groups.
													INT	Length = SmoothingString.InStr(TEXT(","));
													INT SmoothingGroup = (Length != -1) ? appAtoi(*SmoothingString.Left(Length)) : appAtoi(*SmoothingString);

													if( SmoothingGroup && SmoothingGroup <= 32 )
								...
								else if( Section == SECTION_TFACES )
								{
									...
									else
									{
										...
										if(!CollisionGeometry)
										{
											new(MappingChannels(CurrentMappingChannel).FaceTexCoordIdx)INT(BaseTVertexIndex + idx1);
											new(MappingChannels(CurrentMappingChannel).FaceTexCoordIdx)INT(BaseTVertexIndex + idx2);
											new(MappingChannels(CurrentMappingChannel).FaceTexCoordIdx)INT(BaseTVertexIndex + idx3);
										}
									}
								}
