...
struct FPoint2
{
	...
	FPoint2()
	{}
	FPoint2( INT InX, INT InY )
	:	X( InX )
	,	Y( InY )
	{}
	static FPoint2 ZeroValue()
	{
		return FPoint2(0,0);
	}
	static FPoint2 NoneValue()
	{
		return FPoint2(INDEX_NONE,INDEX_NONE);
	}
	...
	UBOOL operator==( const FPoint2& Other ) const
	{
	...
	FPoint2& operator+=( const FPoint2& Other )
	{
		X += Other.X;
		Y += Other.Y;
		return *this;
	...
	FPoint2 operator+( const FPoint2& Other ) const
	{
		return FPoint2(*this) += Other;
	}
	FPoint2 operator-( const FPoint2& Other ) const
	{
		return FPoint2(*this) -= Other;
...
struct FRect
{
	...
	,	Max( X1, Y1 )
	{}
	FRect( FPoint2 InMin, FPoint2 InMax )
	:	Min( InMin )
	,	Max( InMax )
	{}
	FRect( RECT R )
	...
	const FPoint2& operator()( INT i ) const
	{
		return (&Min)[i];
	}
	FPoint2& operator()( INT i )
	{
		return (&Min)[i];
	}
	static INT Num()
	...
	FPoint2 Size()
	{
		return FPoint2( Max.X-Min.X, Max.Y-Min.Y );
	}
	INT Width()
	{
		return Max.X-Min.X;
	...
	FRect& operator+=( const FPoint2& P )
	{
		Min += P;
		Max += P;
		return *this;
	...
	FRect operator+( const FPoint2& P ) const
	{
	...
	FRect Inner( FPoint2 P ) const
	{
		return FRect( Min+P, Max-P );
	}
	UBOOL Contains( FPoint2 P ) const
	{
...
public:
	...
	virtual void SnoopLeftMouseDown( WWindow* Src, FPoint2 P ) {}
	virtual void SnoopRightMouseDown( WWindow* Src, FPoint2 P ) {}
};
...
public FCommandTarget
{
	...
	FPoint2 ClientToScreen( const FPoint2& InP );
	FPoint2 ScreenToClient( const FPoint2& InP );
	FRect ClientToScreen( const FRect& InR );
	FRect ScreenToClient( const FRect& InR );
	FPoint2 GetCursorPos();
	...
	virtual void OnMouseMove( DWORD Flags, FPoint2 Location );
...
class WINDOW_API WThinScrollBar : public WWindow
{
	...
	void OnMouseMove( DWORD Flags, FPoint2 MouseLocation );
	void OnLeftButtonUp();
	void OnDestroy();
	INT OnSetCursor();
	FRect GetThumbRect();
...
class WINDOW_API WCrashBoxDialog : public WDialog
{
	...
	, OkButton  ( this, IDOK, FDelegate(this,(TDelegate)&WCrashBoxDialog::EndDialogTrue) )
...
class WINDOW_API WDragInterceptor : public WWindow
{
	...
	WDragInterceptor()
	{}
	WDragInterceptor( WWindow* InOwner, FPoint2 InDragIndices, FRect InDragClamp, FPoint2 InDrawWidth )
	:	WWindow			( NAME_None, InOwner )
	,	DragIndices		( InDragIndices )
	,	DragPos			( FPoint2::ZeroValue() )
...
public:
	...
	TArray<WComboBox*>		ComboControls;
	...
	virtual void AddClassSelector( const TCHAR* Super, FDelegate Action );
	virtual void AddTemplateSelector( FDelegate Action );
	virtual void OnItemLeftMouseDown( FPoint2 P );
	virtual void OnItemRightMouseDown( FPoint2 P );
	INT GetIndent();
...
public:
	...
	void OnClearButton();
	void OnAddForce();
	void OnAddParticleTemplate();
	void OnAddImpactEffect();
	void OnAddAttachTemplate();
	void OnAddDestroyTemplate();
	void OnAddForceTemplate();
	void OnDeleteForce();
	void OnDeleteParticleTemplate();
	void OnDuplicateParticleTemplate();
	void OnDuplicateForce();
	void OnAddSpecialForce();
	void OnAddAdditionalTemplate();
	void OnDeleteAdditionalTemplate();
	void OnExchangeParticleTemplateClass();
	void OnSortArray();
...
class WINDOW_API WProperties : public WPropertiesBase
{
	...
	void SnoopLeftMouseDown( WWindow* Src, FPoint2 P );
	void SnoopRightMouseDown( WWindow* Src, FPoint2 P );
	void SnoopChar( WWindow* Src, INT Char );
	void SnoopKeyDown( WWindow* Src, INT Char );
	INT GetDividerWidth();
