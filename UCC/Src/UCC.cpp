...
//	GNativeLookupFuncs[20] = &FindUWebUWebRequestNative;
...
#endif
	{
		...
		*CmdLinePtr = 0;
		...
		// Init engine core.
		appInit( TEXT("U2XMP"), CmdLine, &Malloc, &Log, &Error, &Warn, &FileManager, FConfigCacheIni::Factory, 1 ); //!!
			...
			if( Class )
			{
				...
				else
				{
					...
					if( Commandlet->ShowErrorCount )
					{
...
#if 1 //NEW (mdf) hack to show # changed header files when build ends					
						{
						    GWarn->Logf( TEXT("Success - %d error(s), %d warning(s), %d update(s)"), Warn.ErrorCount, Warn.WarningCount, GWarn->WarningCount );                        
						}
#else
                        ...
                        {
#if 1 //NEW (mdf) hack to show # changed header files when build ends					
						    GWarn->Logf( TEXT("Failure - %d error(s), %d warning(s), %d update(s)"), Warn.ErrorCount, Warn.WarningCount, GWarn->WarningCount );
#else
