...
void UTcpNetDriver::TickDispatch( FLOAT DeltaTime )
{
	...
	for( ; ; )
	{
		...
		else
		{
			...
			INT SameIPCount=0;
			for( INT i=0; i<ClientConnections.Num() && !Connection; i++ )
				if( IpAddressMatches( ((UTcpipConnection*)ClientConnections(i))->RemoteAddr, FromAddr ) && ((UTcpipConnection*)ClientConnections(i))->OpenedTime > (appSeconds() - 60.0) )
					SameIPCount++;

			if( SameIPCount < MaxConnPerIPPerMinute )
			{
			...
			if( LogMaxConnPerIPPerMin )
				debugf( TEXT("More than %d logins per minute for %s.  Ignoring."), MaxConnPerIPPerMinute, *IpString(FromAddr.sin_addr) );
		}
	}
	unguard;
}
