...
ToolTipStrings ToolTips_ANIMATION[] = 
{
	...
	//TEXT("Link animation from mesh"), IDMN_EDIT_UNLINKANIM,
...
//
INT Compare( FMyNotifyInfo& A, FMyNotifyInfo& B )
{
...
void WBrowserAnimation::OnCreate()
{
	...
	AnimSeqList->OpenWindow( 1, 0, 0, 0, 1, WS_VSCROLL );
...
void WBrowserAnimation::RefreshViewport()
{
	...
	if( Viewport )
	{
		...
		// Find the index of the current anim
		INT AnimIndex = -1;
		if( !bRefpose )
		{
			USkeletalMeshInstance* MInst = CurrentMeshInstance();
			if( MInst )
			{
				FName AnimName = FindAnimSeqNameFromIndex(AnimSeqList->GetCurrent());
				for( INT a=0; a<MInst->AnimLinkups.Num(); a++ )
				{
					if( MInst->AnimLinkups(a).Anim )
					{
						for( INT i=0; i<MInst->AnimLinkups(a).Anim->AnimSeqs.Num(); i++ )
						{
							FName TestName = MInst->AnimLinkups(a).Anim->AnimSeqs(i).Name;
							if( AnimName == TestName )
								AnimIndex = i;
						}
					}
				}
...
FName WBrowserAnimation::FindAnimSeqNameFromIndex( INT Index )
{
	...
	if( Index >= 0 )
	{
		FString AnimName = AnimSeqList->GetString(Index);
		// chop off the frame count in []
		AnimName = AnimName.Left(AnimName.InStr(TEXT(" [")));
		return *AnimName;
	}
	return NAME_None;
	unguard;
...
FLOAT WBrowserAnimation::GetSeqFramesFromIndex( INT Index )
{
	...
	if( CurrentMeshAnim && ( Index >=0 ) &&  MInst )
	{
		...
		HMeshAnim hAnim = MInst->GetAnimNamed( CurrentSequence );
...
void WBrowserAnimation::OnMeshDelete()
{
	...
	#if 1 //NEW (mdf) Fix from UnProg for "is in use" problem when deleting mesh/animation
	//NOTE (mdf): this doesn't seem to work -- added FORCE hack below
	if( Viewport )
		Viewport->MiscRes = NULL;
	#endif
...
#if 1 //NEW (mdf)	
	appSprintf( l_chCmd, TEXT("DELETE CLASS=SKELETALMESH OBJECT=\"%s\" FORCE"), *Name);
#else
		...
		{			
			for( INT i=0; i<SkelMesh->TagAliases.Num(); i++)
			{
				if( (SkelMesh->TagNames.Num()>i) && (SkelMesh->TagCoords.Num()>i) )
				{
					...
					NewSocket.A_Translation = SkelMesh->TagCoords(i).Origin.TransformVectorBy( SkelMesh->TagCoords(i));
...
void WBrowserAnimation::RefreshNotifiesList()
{
	...
	if( ThisAnim )
	{
		...
		{
			FMyNotifyInfo& Info = EditNotifyProperties->Notifys(EditNotifyProperties->Notifys.AddZeroed());
...
void WBrowserAnimation::RefreshSequenceProperties()
{
	...
	//#TODO: enable OR remove per-sequence rotation/translation type-in editing.
	/*
...
void WBrowserAnimation::SaveMeshProperties()
{
	...
	{		
		...
		// Apply defaultanim if valid AND the current one is unlinked.
		if( ( EditMeshProps->DefaultAnimation != NULL ) && ( SkelMesh->DefaultAnim == NULL ) )
			SkelMesh->DefaultAnim = EditMeshProps->DefaultAnimation;
