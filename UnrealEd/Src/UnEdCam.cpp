...
void UUnrealEdEngine::MouseWheel( UViewport* Viewport, DWORD Buttons, INT Delta)
{
	...
	switch( Viewport->Actor->RendMap )
	{
		...
		case REN_OrthYZ:
		{
			// If travelling upward, use the scaling factor for the next "zoom level" above the current one. 
		if( Delta < 0 ) 
		{
			Viewport->Actor->OrthoZoom += Viewport->Actor->OrthoZoom * ((FLOAT)-Delta * 0.0025);
		}
		else
		{
			Viewport->Actor->OrthoZoom /= ((FLOAT)Delta * 0.0025) + 1.0;
		}

		if( Viewport->Actor->OrthoZoom<MIN_ORTHOZOOM ) 
			Viewport->Actor->OrthoZoom = MIN_ORTHOZOOM; 

		if( Viewport->Actor->OrthoZoom>MAX_ORTHOZOOM ) 
			Viewport->Actor->OrthoZoom = MAX_ORTHOZOOM;
...
void GetMaterialBorderInfo( UMaterial* InMaterial, UBOOL* InDisplayBorder, FColor* InBorderColor, INT* InBorderOffset )
{
	...
	*InBorderColor = FColor(255,255,255);
	...
	if( InMaterial->IsA(UHardwareShader::StaticClass() ) )
	{
		*InDisplayBorder = 1;
		*InBorderColor = FColor(71,190,216);
	}
	if( *InDisplayBorder )
		*InBorderOffset = 8;
}
	...
	if( YL > 0 )
	{
		...
		for( INT i = 0 ; i < n ; i++ )
		{
			...
			if( Y+TextureHeight+LabelHeight+4+BorderOffset>0 && Y<Viewport->SizeY )
			{
				...
				{
					UMaterial* DrawnMaterial = NULL;
					if(ErrMat)
						DrawnMaterial = ErrMat;
					else
						DrawnMaterial = Material;

					FLOAT Scale = GTBOptions->GetScale( Material ),
					USz = DrawnMaterial->MaterialUSize() * Scale,
					VSz = DrawnMaterial->MaterialVSize() * Scale;

					Viewport->Canvas->DrawTile( DrawnMaterial,
						X, Y, USz, VSz,
						0.f, 0.f, DrawnMaterial->MaterialUSize(), DrawnMaterial->MaterialVSize(),
						1.f, FPlane(1.f,1.f,1.f,1.f), FPlane(0,0,0,0));
				}
				else
				{
					// This code will cause the fallback material to be rendered to the texture browser in the event it can't be rendered normally
					UMaterial* TiledMaterial = NULL;
					if(ErrMat)
						TiledMaterial = ErrMat;
					else
						TiledMaterial = Material;

					FLOAT 
						URatio = GTBOptions->GetURatio( TiledMaterial ),
						VRatio = GTBOptions->GetVRatio( TiledMaterial ),
						USz = GTBOptions->GetTileSize( TiledMaterial ),
						VSz = GTBOptions->GetTileSize( TiledMaterial );

					FLOAT TileSize = GTBOptions->GetTileSize( TiledMaterial ),
						USize = TiledMaterial->MaterialUSize(),
						VSize = TiledMaterial->MaterialVSize();
					...
					Viewport->Canvas->DrawTile( TiledMaterial,
						X, Y, USz, VSz,
						0.f, 0.f, TiledMaterial->MaterialUSize() * URatio, TiledMaterial->MaterialVSize() * VRatio,
						1.f, FPlane(1.f,1.f,1.f,1.f), FPlane(0,0,0,0));
				}
...
void DrawMaterial( UViewport* InViewport, UMaterial* InMaterial, FString InName, INT InX, INT& InY, UBOOL bEndofChain, UMaterial* CircRefMaterial )
{
	guard(DrawMaterial);
	...
	// Don't draw material connections if we got circular references at this material.
	if( InMaterial != CircRefMaterial )
	{
		if( Cast<UFinalBlend>( InMaterial ) )
		{
			...
			DrawMaterial( InViewport, Mat->Material, TEXT("Material"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );
		}
		else if( Cast<UShader>( InMaterial ) )
		{
			...
			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->Diffuse, TEXT("Diffuse"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->Opacity, TEXT("Opacity"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->Specular, TEXT("Specular"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->SpecularityMask, TEXT("Specularity Mask"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->SelfIllumination, TEXT("Self Illumination"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->SelfIlluminationMask, TEXT("Self Illumination Mask"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 1 );
			DrawMaterial( InViewport, Mat->Detail, TEXT("Detail"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
		}
		else if( Cast<UTexModifier>( InMaterial ) )
		{
			...
			DrawTreeConnection( InViewport, InX, InY, 1 );
			DrawMaterial( InViewport, Mat->Material, TEXT("Material"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
		}
		else if( Cast<UCombiner>( InMaterial ) )
		{
			...
			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->Material1, TEXT("Material1"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 0 );
			DrawMaterial( InViewport, Mat->Material2, TEXT("Material2"), InX+ArrowWidth+BorderOffset, InY, 0, CircRefMaterial );

			DrawTreeConnection( InViewport, InX, InY, 1 );
			DrawMaterial( InViewport, Mat->Mask, TEXT("Mask"), InX+ArrowWidth+BorderOffset, InY, 1, CircRefMaterial );
		}
...
void DrawMaterialEditor( UViewport* InViewport )
{
	...
	// Check material for circular references.
	UMaterial* CircRefMaterial = NULL;
	TArray<UMaterial*> History;
	if( !Material->CheckCircularReferences(History) )
	{
		INT ErrorIndex = History.Num()-1;
		if( ErrorIndex >= 0 )
			CircRefMaterial = History(ErrorIndex);
	}

	DrawMaterial( InViewport, Material, TEXT(""), X, Y, 0, CircRefMaterial );
...
void DrawAlphaMap( UViewport* Viewport, UMaterial* InAlphaMap, INT InCurX, INT InCurY, UBOOL bAllowForLabel )
{
	...
	//!!MAT
	Viewport->Canvas->DrawTile(Texture, InCurX, InCurY, LAYER_SZ, LAYER_SZ, 0, 0, Texture->MaterialUSize(),Texture->MaterialVSize(), 0, FColor(255,255,255), FPlane(0,0,0,0));
...
void DrawLayer( UViewport* Viewport, ATerrainInfo* InTerrainInfo, INT InLayer, INT CurX, INT CurY )
{
	...
	if( InTerrainInfo->Layers[InLayer].Texture && InTerrainInfo->Layers[InLayer].AlphaMap )
	{
		Viewport->Canvas->CurX = CurX+LAYER_SZ+LAYER_SZ+8;
		Viewport->Canvas->CurY = CurY;
		if( InTerrainInfo->Layers[InLayer].AlphaMap )
			Viewport->Canvas->WrappedPrintf( Viewport->Canvas->SmallFont, 0, *FString::Printf(TEXT("Alpha: %s"), 
				InTerrainInfo->Layers[InLayer].AlphaMap->GetName()));

		Viewport->Canvas->CurX = CurX+LAYER_SZ+LAYER_SZ+8 + 165;
		Viewport->Canvas->CurY = CurY;
		if( InTerrainInfo->Layers[InLayer].Texture )
			Viewport->Canvas->WrappedPrintf( Viewport->Canvas->SmallFont, 0, *FString::Printf(TEXT("Tex: %s"), 
				InTerrainInfo->Layers[InLayer].Texture->GetName()));

		Viewport->Canvas->CurX = CurX+LAYER_SZ+LAYER_SZ+8;
		Viewport->Canvas->CurY = CurY + 15;
		if( InTerrainInfo->Layers[InLayer].AlphaMap )
			Viewport->Canvas->WrappedPrintf( Viewport->Canvas->SmallFont, 0, *FString::Printf(TEXT("Size: %dx%d"), 
				InTerrainInfo->Layers[InLayer].AlphaMap->UClamp,InTerrainInfo->Layers[InLayer].AlphaMap->VClamp));

		Viewport->Canvas->CurX = CurX+LAYER_SZ+LAYER_SZ+8 + 165;
		Viewport->Canvas->CurY = CurY + 15;
		Viewport->Canvas->WrappedPrintf( Viewport->Canvas->SmallFont, 0, *FString::Printf(TEXT("Scale: %1.1fx%1.1f"),
			InTerrainInfo->Layers[InLayer].UScale, InTerrainInfo->Layers[InLayer].VScale));
	}
	else
	{
...
void DrawMatineeSubActions( UViewport* Viewport )
{
	...
	for( INT x = 0 ; x < MA->SubActions.Num() ; ++x )
	{
		...
		else if( Cast<USubActionFOV>(SubAction) )
		{
			...
			CurY += FontHeight + FontHeight;
			DrawField( Viewport, CurX, CurY, FColor(192,192,192), TEXT("FOV : "), FColor(0,255,255), TEXT("%1.1f / %1.1f"), SAF->FOV.A, SAF->FOV.B );
			CurY += FontHeight;
		}
		else if( Cast<USubActionGameSpeed>(SubAction) )
		{
			...
			CurY += FontHeight + FontHeight;
			DrawField( Viewport, CurX, CurY, FColor(192,192,192), TEXT("Speed : "), FColor(0,255,255), TEXT("%1.1f / %1.1f"), SAGS->GameSpeed.A, SAGS->GameSpeed.B );
			CurY += FontHeight;
		}
		...
		else if( Cast<USubActionParticleTrigger>(SubAction) )
		{
			USubActionParticleTrigger* SAPT = Cast<USubActionParticleTrigger>(SubAction);
			CurY += FontHeight + FontHeight;
			DrawField( Viewport, CurX, CurY, FColor(192,192,192), TEXT("Name : "), FColor(0,255,255), TEXT("%s"), *SAPT->Name );
			CurY += FontHeight;
		}
		else if( Cast<USubActionConsoleCommand>(SubAction) )
		{
			USubActionConsoleCommand* SACC = Cast<USubActionConsoleCommand>(SubAction);
			CurY += FontHeight + FontHeight;
			DrawField( Viewport, CurX, CurY, FColor(192,192,192), TEXT("Command : "), FColor(0,255,255), TEXT("%s"), SACC->Command );
			CurY += FontHeight;
		}
		else if( Cast<USubActionSceneSpeed>(SubAction) )
		{
			...
			CurY += FontHeight + FontHeight;
			DrawField( Viewport, CurX, CurY, FColor(192,192,192), TEXT("Speed : "), FColor(0,255,255), TEXT("%1.1f / %1.1f"), SASS->SceneSpeed.A, SASS->SceneSpeed.B );
			CurY += FontHeight;
		}
	...
	if(Viewport->Lock(HitData,HitCount))
	{
		...
		switch( Actor->RendMap )
		{
			...
			case REN_MeshView:
			{
				...
				// Old vertex-animation-only mesh viewer.
				// TODO: reorganize just like Animation (=Skeletal mesh) browser.
				UMesh* Mesh = CurrentMesh; //GBrowserMesh->WorkMesh;

				// Update in renderer - so mesh last displayed is the one used when a 'use current' button is pressed..
				// CurrentMesh = GBrowserMesh->WorkMesh;
			...
			default:
			{
				...
				if( Viewport->Actor->ShowFlags & SHOW_Coords )
				{
					Viewport->Canvas->CurX = 0;
					Viewport->Canvas->CurY = 0;
					Viewport->Canvas->Color = FColor(255,255,255);

					FString Text = *FString::Printf( TEXT("%1.1f, %1.1f, %1.1f"), GSnappedLocation.X, GSnappedLocation.Y, GSnappedLocation.Z );
