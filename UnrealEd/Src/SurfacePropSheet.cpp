...
void WPageFlags::OpenWindow( INT InDlgId, HMODULE InHMOD )
{
	...
	InvisibleCheck->OpenWindow( 1, 0, 0, 0, 0, TEXT(""), 0 );
...
void WPageFlags::OnButtonClicked()
{
	...
	INT Checked = SendMessageX( hwndButton, BM_GETCHECK, 0, 0 );
	if( Checked == BST_CHECKED )
		SendMessageX( hwndButton, BM_SETCHECK, BST_CHECKED, 0 );
	else
		SendMessageX( hwndButton, BM_SETCHECK, BST_UNCHECKED, 0 );
...
void WSurfacePropSheet::OnCreate()
{
	...
	FlagsPage->OpenWindow( IDPP_SP_FLAGS1, GetModuleHandleA("unrealed.exe") );
	PropSheet->AddPage( FlagsPage );

	PanRotScalePage = new WPagePanRotScale( PropSheet->Tabs );
	PanRotScalePage->OpenWindow( IDPP_SP_PANROTSCALE, GetModuleHandleA("unrealed.exe") );
	PropSheet->AddPage( PanRotScalePage );

	AlignmentPage = new WPageAlignment( PropSheet->Tabs );
	AlignmentPage->OpenWindow( IDPP_SP_ALIGNMENT, GetModuleHandleA("unrealed.exe") );
	PropSheet->AddPage( AlignmentPage );

	StatsPage = new WPageStats( PropSheet->Tabs );
	StatsPage->OpenWindow( IDPP_SP_STATS, GetModuleHandleA("unrealed.exe") );
