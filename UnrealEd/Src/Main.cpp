...
void RefreshEditor( INT InFlags )
{
	...
	if( InFlags&ERefreshEditor_ParticleBrowser ) GBrowserMaster->RefreshBrowser( eBROWSER_PARTICLES );
...
void FileSaveChanges( HWND hWnd )
{
	...
	if( GLevelFrame 
			&& GUnrealEd->Trans->CanUndo() )
	{
		TCHAR l_chMsg[512];

		appSprintf( l_chMsg, TEXT("Save changes to %s?"), appStrlen( GLevelFrame->GetMapFilename() ) ? GLevelFrame->GetMapFilename() : TEXT("Untitled") );
...
class WEditorFrame : public WMdiFrame, public FNotifyHook
{
	...
	UBOOL OnClose()
	{
		...
		SAFEDELETENULL(GBrowserParticles);
	...
	void OnCommand( INT Command )
	{
		...
		switch( Command )
		{
			...
			case ID_FILE_EXPORT:
			{
				...
				ofn.lpstrFile = File;
				ofn.nMaxFile = sizeof(char) * 8192;
				ofn.lpstrFilter = "Supported formats (*.t3d,*.stl)\0*.t3d;*.stl\0Unreal Text (*.t3d)\0*.t3d\0Stereo Litho (*.stl)\0*.stl\0All Files\0*.*\0\0";
				ofn.lpstrInitialDir = appToAnsi( *(GLastDir[eLASTDIR_UNR]) );
				ofn.lpstrDefExt = "t3d";
			...
			case ID_ResetParticles:
			{
				if( GUnrealEd && GUnrealEd->Level )
				{
					GUnrealEd->Exec( TEXT("ACTOR PARTICLESYSTEMS CLEAN") );
					GUnrealEd->RedrawLevel( GUnrealEd->Level );
				}
			}
			break;
			...
			case IDMN_MRU8:
			{
				// See if the file exists before trying to load it.
			...
			{
				GBrowserMaster->ShowBrowser(eBROWSER_PARTICLES);
				if( GBrowserParticles->IsDocked() ) GBrowserMaster->Show(1);
			}
			break;
			...
			case WM_BROWSER_DOCK:
			{
				...
				switch( Browsr )
				{
					...
					case eBROWSER_PARTICLES:
						SAFEDELETENULL(GBrowserParticles);
						GBrowserParticles = new WBrowserParticles( TEXT("Particle Browser"), GBrowserMaster, GEditorFrame->hWnd );
						check(GBrowserParticles);
						GBrowserParticles->OpenWindow( 1 );
						GBrowserMaster->ShowBrowser(eBROWSER_PARTICLES);
						break;
				}
				unguard;
			...
			case WM_BROWSER_UNDOCK:
			{
				...
				switch( Browsr )
				{
					...
					case eBROWSER_PARTICLES:
						SAFEDELETENULL(GBrowserParticles);
						GBrowserParticles = new WBrowserParticles( TEXT("Particle Browser"), GEditorFrame, GEditorFrame->hWnd );
						check(GBrowserParticles);
						GBrowserParticles->OpenWindow( 0 );
						GBrowserMaster->ShowBrowser(eBROWSER_PARTICLES);
						break;
				}
			...
			case WM_EDC_USECURRENT:
			{
				...
				else if( !appStrcmp( *GetPropResult, TEXT("ParticleGenerator") ) )
				{
					if( GBrowserParticles )
						Cur = *GBrowserParticles->GetCurrentPathName();
				}
				else
				{
			...
			case WM_EDC_RTCLICKPOLY:
			{
				...
				if( GUnrealEd->CurrentParticleSystem )
				{
					sprintf( Buffer, "&Add Particle System: '%s'", TCHAR_TO_ANSI( GUnrealEd->CurrentParticleSystem->GetName() ) );
					mif.dwTypeData = Buffer;
					SetMenuItemInfoA( submenu, ID_BackdropPopupAddParticleSystemHere, FALSE, &mif );
				}
				else
					DeleteMenu( submenu, ID_BackdropPopupAddParticleSystemHere, MF_BYCOMMAND );
			...
			case WM_EDC_RTCLICKACTORSTATICMESH:
			{
				...
				if( Command == WM_EDC_RTCLICKACTORSTATICMESH)
				{
					...
					if( GUnrealEd->CurrentParticleSystem )
					{
						sprintf( Buffer, "&Add Particle System: '%s'", TCHAR_TO_ANSI( GUnrealEd->CurrentParticleSystem->GetPathName() ) );
						mif.dwTypeData = Buffer;
						SetMenuItemInfoA( submenu, ID_BackdropPopupAddParticleSystemHere, FALSE, &mif );
					}
					else
			...
			case ID_BackdropPopupAddParticleSystemHere:
			{
				if( GUnrealEd->CurrentParticleSystem )
				{
					FString S = GUnrealEd->CurrentParticleSystem->GetFullName();
					GUnrealEd->Exec( *(FString::Printf( TEXT("ACTOR PARTICLESYSTEMS NEW IMAGE=%s"), S.Mid(S.InStr(TEXT(" "))+1) ) ) );
					GUnrealEd->Exec( TEXT("POLY SELECT NONE") );
				}
			}
			break;
			...
			case IDMENU_ActorPopupLockMovement:
			{
				GUnrealEd->Exec( TEXT("ACTOR TOGGLE LOCKMOVEMENT") );
			}
			break;

			case IDMENU_ActorAutoAlignTerrain:
			{
				GUnrealEd->Exec( TEXT("ACTOR TOGGLE AUTOALIGNTERRAIN") );
			}
			break;
...
INT WINAPI WinMain( HINSTANCE hInInstance, HINSTANCE hPrevInstance, char* InCmdLine, INT nCmdShow )
{
	...
	try
	{
		...
		appInit( TEXT("u2xmp"), GetCommandLine(), &Malloc, &Log, &Error, &Warn, &FileManager, FConfigCacheIni::Factory, 1 );
		...
		IMPLEMENT_WINDOWCLASS(WBrowserParticles,CS_DBLCLKS | CS_VREDRAW | CS_HREDRAW);
		...
		if( !ShowLog && !ParseParam(appCmdLine(),TEXT("server")) )
			InitSplash( TEXT("..\\Help\\EdSplash.bmp") );
		...
		// UnrealEd INI.
		FString MyEdIni=TEXT("MyUnrealEd.ini");
		if( GFileManager->FileSize(*MyEdIni)<0 )
		{
			FString S=TEXT("=== Custom settings ===\n");
			appSaveStringToFile( S, *MyEdIni );
		}
		MakeWriteable( *MyEdIni );
		...
		GLastDir[eLASTDIR_UPS] = TEXT("..\\particles");
		...
		if( !GConfig->GetString( TEXT("URL"), TEXT("MapExt"), GMapExt ) )		GMapExt = TEXT("unr");
		...
		GBrowserMaster->Browsers[eBROWSER_PARTICLES] = (WBrowser**)(&GBrowserParticles);
		...
		if(!GConfig->GetInt( TEXT("Particle Browser"), TEXT("Docked"), bDocked, TEXT("UnrealEd.ini") ))	bDocked = FALSE;
		SendMessageX( GEditorFrame->hWnd, WM_COMMAND, bDocked ? WM_BROWSER_DOCK : WM_BROWSER_UNDOCK, eBROWSER_PARTICLES );
		if( !bDocked ) 
		{
			if(!GConfig->GetInt( *GBrowserParticles->PersistentName, TEXT("Active"), bActive, TEXT("UnrealEd.ini") ))	bActive = FALSE;
			GBrowserParticles->Show( bActive );
		}
