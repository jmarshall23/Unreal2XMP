		...
		if( dlg.DoModal( TEXT("") ) )
		{
			...
			else
			{
				...
				if( !GEditor->StaticFindObject( UTexture::StaticClass(), Pkg, *Proxy->Name ) )
				{
					...
					appMsgf(0, TEXT("The heightmap texture '%s' could not be created because a texture with this name already exists."), *Proxy->Name );
			...
			if( dlg.DoModal( TEXT("") ) )
			{
				...
				else
				{
					...
					if( !GEditor->StaticFindObject( UTexture::StaticClass(), Pkg, *Proxy->Name ) )
					{
						...
						for( INT x = 0 ; x < Result->USize ; ++x )
							for( INT y = 0 ; y < Result->VSize ; ++y )
								*((FColor*)&Result->Mips(0).DataArray( 4*(x + y*Result->USize))) = FColor(Proxy->ColorFill.R,Proxy->ColorFill.G,Proxy->ColorFill.B, GTerrainTools.CurrentLayer == 1 ? 255 : Proxy->AlphaFill);
						...
						appMsgf(0, TEXT("The alphamap texture '%s' could not be created because a texture of this name already exists."), *Proxy->Name );
				}
			}
		}
		unguard;
...
class WPageToolsDecoLayers : public WPropertyPage
{
	...
	void OnDelete()
	{
		...
		{
			if (GTerrainTools.CurrentLayer-32 >= TI->DecoLayers.Num())
				return;
...
class WTerrainToolsSheet : public WWindow
{
	...
	void OnCreate()
	{
		...
		ToolsTerrainsPage->OpenWindow( IDPP_TE_TOOLS_TERRAINS, GetModuleHandleA("unrealed.exe") );
		ToolsSheet->AddPage( ToolsTerrainsPage );

		ToolsLayersPage = new WPageToolsLayers( ToolsSheet->Tabs );
		ToolsLayersPage->OpenWindow( IDPP_TE_TOOLS_LAYERS, GetModuleHandleA("unrealed.exe") );
		ToolsSheet->AddPage( ToolsLayersPage );

		ToolsDecoLayersPage = new WPageToolsDecoLayers( ToolsSheet->Tabs );
		ToolsDecoLayersPage->OpenWindow( IDPP_TE_TOOLS_DECORATIONS, GetModuleHandleA("unrealed.exe") );
...
class WTerrainEditSheet : public WWindow
{
	...
	void OnCreate()
	{
		...
		ToolsPage->OpenWindow( IDPP_TE_TOOLS, GetModuleHandleA("unrealed.exe") );
		PropSheet->AddPage( ToolsPage );

		MiscPage = new WPageMisc( PropSheet->Tabs );
		MiscPage->OpenWindow( IDPP_TE_MISC, GetModuleHandleA("unrealed.exe") );
