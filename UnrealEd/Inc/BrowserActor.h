		...
		for( TObjectIterator<UClass> It ; It ; ++It )
			if( It->GetSuperClass() == InParent )
			{
				FString Name = FString::Printf(TEXT("%s%s"),
					(It->ClassFlags & CLASS_Placeable) ? TEXT("*") : TEXT(" "),
					It->GetName()
				);

				if( bPlaceableOnly && !(It->ClassFlags & CLASS_Placeable) && !HasChildren( *It, 1 ) )
					continue;

				TreeView->AddItem( *Name, InHTI, HasChildren( *It, bPlaceableOnly ) );
			}
