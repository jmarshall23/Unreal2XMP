...
class WDlgImportTexture : public WDialog
{
	...
	WComboBox PackageEdit; // Changed by Demiurge (PackagePicker)
	WComboBox GroupEdit; // Changed by Demiurge (PackagePicker)
	WEdit NameEdit;
	WCheckBox MaskedCheck;
	WCheckBox MipMapCheck;
	WCheckBox DetailHackCheck;
	WCheckBox BumpMapCheck;
	WComboBox CompressionCombo;
	...
	,   BumpMapCheck    ( this, IDCK_BUMPMAP )
	,	DetailHackCheck	( this, IDCK_DETAIL_HACK )
	,	CompressionCombo( this, IDCB_COMPRESSION )
	{
	}
	...
	void OnInitDialog()
	{
		...
		CompressionCombo.AddString( TEXT("None") );
		CompressionCombo.AddString( TEXT("DXT1") );
		CompressionCombo.AddString( TEXT("DXT3") );
		CompressionCombo.AddString( TEXT("DXT5") );
		CompressionCombo.SetCurrent(0);

		// Added by Demiurge (PackagePicker)

		// Stolen from RefreshPackage and tweaked-out
		...
		{
			PackageEdit.AddString( *(StringArray(x)) );
			if(StringArray(x) == defPackage)
			{
				Current = x;
			}
		}
		PackageEdit.SetCurrent( PackageEdit.FindString(*defPackage) );
		RefreshGroupCombo();
		PackageEdit.AddString(TEXT("<Type New>"));
		PackageEdit.SelectionEndOkDelegate = FDelegate(this, (TDelegate)&WDlgImportTexture::RefreshGroupCombo);
		// End add by Demiurge
	...
	{
		// Stolen from RefreshGroups
		FString Package = PackageEdit.GetString( PackageEdit.GetCurrent() );
		...
		for( INT x = 0 ; x < StringArray.Num() ; ++x )
			GroupEdit.AddString( *(StringArray(x)) );


		GroupEdit.SetCurrent( GroupEdit.FindString(*defGroup) );
		GroupEdit.AddString(TEXT("<Type New>"));
		StringArray.Empty();
	}
	// End add by Demiurge
	...
	void ImportFile( FString Filename )
	{
		...
		if( Group.Len() )
			appSprintf( l_chCmd, TEXT("TEXTURE IMPORT FILE=\"%s\" NAME=\"%s\" PACKAGE=\"%s\" GROUP=\"%s\" MIPS=%d MASKED=%d"),
				*(*Filenames)(iCurrentFilename), *Name, *Package, *Group,
				MipMapCheck.IsChecked(), MaskedCheck.IsChecked() );
		else
			appSprintf( l_chCmd, TEXT("TEXTURE IMPORT FILE=\"%s\" NAME=\"%s\" PACKAGE=\"%s\" MIPS=%d MASKED=%d"),
	...
	void RefreshTextureList( void )
	{
		...
		if( GroupAllCheck->IsChecked() )
		{
			appSprintf( l_chCmd, TEXT("CAMERA UPDATE FLAGS=%d MISC2=%d REN=%d NAME=TextureBrowser PACKAGE=\"%s\" GROUP=\"%s\""),
				SHOW_StandardView | SHOW_ChildWindow | SHOW_NoFallbackMaterials,
				iScroll,
				REN_TexBrowser,
				*Package,
				TEXT("(All)") );
...
class WBrowserTexture : public WBrowser
{
	...
	void OnCreate()
	{
		...
		MaterialsPage->OpenWindow( IDPP_TB_MATERIALS, GetModuleHandleA("unrealed.exe") );
		MaterialsPage->PropSheet = PropSheet;
		PropSheet->AddPage( MaterialsPage );

		UsedPage = new WPageUsed( PropSheet->Tabs );
		UsedPage->OpenWindow( IDPP_TB_USED, GetModuleHandleA("unrealed.exe") );
		UsedPage->PropSheet = PropSheet;
		PropSheet->AddPage( UsedPage );

		MRUPage = new WPageMRU( PropSheet->Tabs );
		MRUPage->OpenWindow( IDPP_TB_MRU, GetModuleHandleA("unrealed.exe") );
	...
	void OnCommand( INT Command )
	{
		...
		switch( Command )
		{
				...
				for( TArray<AActor*>::TIterator ItA(GUnrealEd->Level->Actors); ItA; ++ItA )
				{
					...
					if( Actor )
					{
						...
						if(TerrainInfo)
						{
							...
							for(INT i = 0; i < 32; i++)
							{
								UBOOL AlreadyPrompted = false;
								UBOOL Prompt = false;
								// if that layer is using the texture.
								if(TerrainInfo->Layers[i].Texture == GUnrealEd->CurrentMaterial)
								{
									// prompt the user if we haven't already
									if(!AlreadyPrompted)
									{
										Prompt = appMsgf(1, TEXT("Texture exists in terrain. Are you sure you want to remove it?"));
										AlreadyPrompted = true;
									}
									if(Prompt)
									{
										// remove it
										TerrainInfo->Layers[i].Texture = TerrainInfo->Level->DefaultTexture;
									}
								}
							}
						}
						...
						if(StaticMeshActor)
						{
							// Get the static mesh itself.
							UStaticMesh* Mesh = StaticMeshActor->StaticMesh;

							if(Mesh)
							{
								// For each material in the mesh
								for(TArray<FStaticMeshMaterial>::TIterator It(Mesh->Materials); It; ++It)
								{
									UBOOL AlreadyPrompted = false;
									UBOOL Prompt = false;
									// if that layer is using the texture.
									if(It->Material == GUnrealEd->CurrentMaterial)
									{
										// prompt the user if we haven't already
										if(!AlreadyPrompted)
										{
											Prompt = appMsgf(1, TEXT("Texture exists in %s which is currently in use. Do you want to remove it from that static mesh?"), Mesh->GetFullName());
											AlreadyPrompted = true;
										}
										if(Prompt)
										{
											// remove it
											It->Material = StaticMeshActor->Level->DefaultTexture;
										}
									}
								}
							}
						...
						for( TObjectIterator<UShader> It; It; ++It )
						{
							...
							UBOOL Prompt = false;
							// if that layer is using the texture.
							if(	It->Diffuse == GUnrealEd->CurrentMaterial ||
								It->Opacity == GUnrealEd->CurrentMaterial ||
								It->Specular == GUnrealEd->CurrentMaterial ||
                                It->SpecularityMask == GUnrealEd->CurrentMaterial ||
								It->SelfIllumination == GUnrealEd->CurrentMaterial ||
                                It->SelfIlluminationMask == GUnrealEd->CurrentMaterial ||
                                It->Detail == GUnrealEd->CurrentMaterial )
							{
								// prompt the user if we haven't already
								if(!AlreadyPrompted)
								{
									Prompt = appMsgf(1, TEXT("Texture is being used by %s. Do you want to remove it from that shader?"), It->GetFullName());
									AlreadyPrompted = true;
								}
								if(Prompt)
								{
									if(	It->Diffuse == GUnrealEd->CurrentMaterial )
									{
										It->Diffuse = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
									if(	It->Opacity == GUnrealEd->CurrentMaterial )
									{
										It->Opacity = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
									if(	It->Specular == GUnrealEd->CurrentMaterial )
									{
										It->Specular = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
									if(	It->SpecularityMask == GUnrealEd->CurrentMaterial )
									{
										It->SpecularityMask = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
									if(	It->SelfIllumination == GUnrealEd->CurrentMaterial )
									{
										It->SelfIllumination = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
									if(	It->SelfIlluminationMask == GUnrealEd->CurrentMaterial )
									{
										It->SelfIlluminationMask = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
									if(	It->Detail == GUnrealEd->CurrentMaterial )
									{
										It->Detail = GUnrealEd->Level->GetLevelInfo()->DefaultTexture;
									}
								}
							}
						}
			...
			case IDMN_TB_IMPORT_SHADER:
			{
				...
				ofn.lpstrFile = File;
				ofn.nMaxFile = sizeof(char) * 8192;
				ofn.lpstrFilter = "Supported formats (*.xml)\0*.xml\0Shader Files (*.xml)\0*.xml\0All Files\0*.*\0\0";
				ofn.nFilterIndex = 1;
				ofn.lpstrDefExt = "xml";
				...
				if( GetOpenFileNameA(&ofn) )
				{
					...
					WDlgImportShader l_dlg( NULL, this );
					if( l_dlg.DoModal( Package, Group, &FilenamesArray ) )
					{
					// Flip to the texture/group that was used for importing
					GBrowserMaster->RefreshAll();
	...
	virtual void SetCaption( FString* Tail = NULL )
	{
		...
		if( GUnrealEd->CurrentMaterial )
		{
			...
			if( Cast<UBitmapMaterial>(GUnrealEd->CurrentMaterial) )
				Extra = *FString::Printf( TEXT("%s (%dx%d)"), Extra, Cast<UBitmapMaterial>(GUnrealEd->CurrentMaterial)->USize, Cast<UBitmapMaterial>(GUnrealEd->CurrentMaterial)->VSize );
		}
