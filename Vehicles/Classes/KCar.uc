...
simulated function PostNetBeginPlay()
{
	...
	frontLeft.ConglomerateID=self;
	frontRight.ConglomerateID=self;
	rearLeft.ConglomerateID=self;
	rearRight.ConglomerateID=self;
...
simulated event Destroyed()
{
	...
	if (frontLeft? && frontLeft.WheelJoint?)
		frontLeft.WheelJoint.Destroy();
	if (frontRight? && frontRight.WheelJoint?)
		frontRight.WheelJoint.Destroy();
	if (rearLeft? && rearLeft.WheelJoint?)
		rearLeft.WheelJoint.Destroy();
	if (rearRight? && rearRight.WheelJoint?)
...
simulated event KApplyForce(out vector Force, out vector Torque)
{
	...
	Force = worldForward * SprintForce;
}

function ServerSprint()
{
	// if we are already sprinting - dont do it again!
	if (Role == ROLE_Authority)
		SprintTimeLeft += SprintTime * (Stamina / StaminaMax); // Start the sprint on the server
	Stamina=0;
}
...
simulated function DoSprint()
{
	...
	if (level.netmode != NM_DedicatedServer)
		PlaySound(SprintSound, SLOT_None, 255, , 128 );
	ServerSprint();
}
...
function ProcessCarInput()
{
	...
	if( PassengerControllers[0] == None )
	{
		if(bAutoDrive == true)
		{
			Gear = 1;
	...
	else
	{
		...
		else if(Throttle < -0.01) // pressing backwards
		{
			...
			if(ForwardVel > StopThreshold && Gear != -1) // going forward - so brake first
			{
				//Log("F - Brake");
				Gear = 0;
				OutputBrake = true;
			...
			else // stopped or going backward, so drive
			{
				//Log("F - Drive");
				Gear = -1;
				OutputBrake = false;
...
/*			// We have to release the brakes and then press reverse again to go into reverse
		...
		} else // not pressing either
		{
			...
			if (bThrottling && !bOldThrottling && level.NetMode!= NM_DedicatedServer && Self.IsLocallyControlled())
			{
...
simulated function Tick(float Delta)
{
	...
	{
		ProcessCarInput();
		if (Throttle != 0 && Online)
			EnergyAccumulator+=Delta;
		if (EnergyAccumulator >=1)
		{
			XMPGame(Level.Game).DrainTeamEnergy( GetTeam(), (EnergyAccumulator/1000) , 'DrivingVehicle' );
			EnergyAccumulator -=1;
		}
		PackState();
	}
    ...
    // Motor
	//PWC 2003.04.01 Change so that TorqueSplit is reversed when car is in reverse
	if (Gear > -1)
	{
		// FRONT
	...
	{
		// FRONT
	    frontLeft.WheelJoint.KMotorTorque = 0.5 * OutputTorque * TorqueSplit;
	    frontRight.WheelJoint.KMotorTorque = 0.5 * OutputTorque * TorqueSplit;

		// REAR
	    rearLeft.WheelJoint.KMotorTorque = 0.5 * OutputTorque * (1-TorqueSplit);
	    rearRight.WheelJoint.KMotorTorque = 0.5 * OutputTorque * (1-TorqueSplit);
	}

    // Braking

	if(OutputBrake || (Gear == 0 && EngineBraking == true))
	{
	...
	if(SprintTimeLeft > 0)
	{
		...
		SprintTimeLeft = FMax(SprintTimeLeft, 0.0); // Make sure it doesn't go negative
	}
