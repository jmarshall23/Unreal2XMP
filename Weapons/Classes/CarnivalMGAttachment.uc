class CarnivalMGAttachment extends U2WeaponAttachment;
...
simulated event ThirdPersonEffects()
{
    ...
    if (Level.NetMode == NM_Client && Instigator? && !(Instigator.IsLocallyControlled()))
	{
		...
		if (!MuzzleFlashA)
		{
			MuzzleFlashA = class'ParticleGenerator'.static.CreateNew( self, ParticleSalamander'Turret_Post_FX.ParticleSalamander0', Location  );
		}
		if (!MuzzleFlashB)
		{
			MuzzleFlashB = class'ParticleGenerator'.static.CreateNew( self, ParticleSalamander'Turret_Post_FX.ParticleSalamander0', Location  );
		}
		if (!FireA)
		{
			FireA = class'ParticleGenerator'.static.CreateNew( self, ParticleSalamander'Turret_Post_FX.ParticleSalamander3', Location  );
		}
		if (!FireB)
		{
			FireB = class'ParticleGenerator'.static.CreateNew( self, ParticleSalamander'Turret_Post_FX.ParticleSalamander3', Location  );
		}
