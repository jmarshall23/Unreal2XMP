...
public:
	...
	, Quotes( 0 )
	{}
	void Read( const TCHAR* Filename, UBOOL Clean=1 )
	{
	...
	UBOOL Write( const TCHAR* Filename, FConfigFile* Defaults=NULL )
	{
		...
		if( Defaults )
		{
			// only write out diffs.
			for( TIterator It(*this); It; ++It )
			{
				FConfigSection* DefSection = Defaults->Find( It.Key() );
				...
				for( FConfigSection::TIterator It2(It.Value()); It2; ++It2 )
				{
					if( DefSection )
					{
						...
						DefSection->MultiFind( It2.Key(), Values );
						for(INT i=0;i<Values.Num();i++)
							if( It2.Value()==Values(i) )
								goto SkipValue;
					}
			...
			FString MyFilename=TEXT("My"); MyFilename+=Filename;
			return appSaveStringToFile( Text, *MyFilename );
		}
		else
		{
			for( TIterator It(*this); It; ++It )
...
public:
	...
	FConfigFile* Find( const TCHAR* InFilename, UBOOL CreateIfNotFound, UBOOL ForceReload=0 )
	{
		...
		if( Len<5 || (Filename[Len-2]!='.' && Filename[Len-3]!='.' && Filename[Len-4]!='.' && Filename[Len-5]!='.') )
		...
		// Get file.
		FConfigFile* Result = ForceReload ?  NULL : TMap<FString,FConfigFile>::Find( Filename );
		if( !Result && (CreateIfNotFound || GFileManager->FileSize(Filename)>=0)  )
		{
			...
			// check for overrides.
			FString MyFilename=TEXT("My"); MyFilename+=Filename;
			if( GFileManager->FileSize(*MyFilename)>=0 )
			{
				// save off defaults.
				FString DefFilename=TEXT("Def"); DefFilename+=Filename;
				Set( *DefFilename, *Result );

				// read in overrides.
				Result->Read( *MyFilename, 0 );
			}
		}
	...
	void Flush( UBOOL Read, const TCHAR* Filename=NULL )
	{
		...
		for( TIterator It(*this); It; ++It )
		{
			...
			{
				// check for defaults.
				FString DefFilename=TEXT("Def"); DefFilename+=It.Key();
				FConfigFile* Defaults = TMap<FString,FConfigFile>::Find( DefFilename );

				It.Value().Write( *It.Key(), Defaults );
			}
		}
		if( Read )
		{
	...
	TMultiMap<FString,FString>* GetSectionPrivate( const TCHAR* Section, UBOOL Force, UBOOL Const, const TCHAR* Filename, UBOOL ReloadFile=0 )
	{
		guard(FConfigCacheIni::GetSectionPrivate);
		FConfigFile* File = Find( Filename, Force, ReloadFile );
		if( !File )
			return NULL;
		FConfigSection* Sec = File->Find( Section );
		if( !Sec && Force )
