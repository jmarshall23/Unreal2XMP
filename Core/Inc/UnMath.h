...
const FLOAT OneOver128			= 0.0078125f;
const FLOAT RadiansToDegrees	= 180.0f / (FLOAT)PI;
const FLOAT DegreesToRadians	= (FLOAT)PI / 180.0f;
...
public:
	...
	FVector Inverse() const
	{
		return FVector(1.f/X,1.f/Y,1.f/Z);
	}
	...
	{
		return FString::Printf(TEXT("vect(%f,%f,%f)"),X,Y,Z);
	}
...
public:
	...
	CORE_API friend FArchive& operator<<(FArchive& Ar, CVector& T)
	{
...
public:
	...
	friend FRange operator/( FLOAT Scale, const FRange& R )
	{
		return FRange( Scale / R.A, Scale / R.B );
	}
	FLOAT Blend( FLOAT V ) const
	{
		return appBlend( A, B, V );
	}
	FLOAT Ablend( FLOAT V ) const
	{
		return appAblend( A, B, V );
	}
...
public:
	...
	FString String() const
	{
		return FString::Printf(TEXT("FBox: Min=%s Max=%s IsValid=%s"),*Min.String(),*Max.String(),IsValid?TEXT("True"):TEXT("False"));
	}
...
public:
	...
	CORE_API friend FArchive& operator<<(FArchive& Ar, CBox& T)
	{
...
{
    FLOAT max_pitch = Clamp<FLOAT>(spread_degrees * (PI / 180.0f / 2.0f),0.0f,180.0f);
...
CORE_API inline FLOAT Det2x2( FLOAT A, FLOAT B, FLOAT C, FLOAT D )
{
	return ( A*D - B*C );
}

CORE_API inline FLOAT Det3x3( 
...
/*-----------------------------------------------------------------------------
Returns true if the given point is on the polygon defined by the vertices in
Points. The polygon is assumed to be convex and its vertices in the same plane
and the given point is assumed to be in that plane also. PolyNormal is the 
polygon's normal.
*/

inline UBOOL FPointConvexPolygonIntersection( const FVector& PlanePoint, const TArray<FVector> Points, const FVector& PolyNormal )
{
	...
	for( INT ii=0 ; ii<Points.Num(); ii++ )
	{
		...
		Side = Points( ii ) - Points( (ii-1 < 0 ) ? Points.Num()-1 : ii-1 );
		SidePlaneNormal = Side ^ PolyNormal;
		SidePlaneNormal.Normalize();

		// If point is not behind all the planes created by this polys edges, it's outside the poly.
		if( FPointPlaneDist( PlanePoint, Points( ii ), SidePlaneNormal ) > THRESH_POINT_ON_PLANE )
			return 0;
	}
...
inline UBOOL FLineConvexPolygonIntersection
(
	...
	FVector& IntersectionPoint )
{
	if( Points.Num() >= 3 )
	{
		...
		FVector PolyNormal = ((Points(0) - Points(1)) ^ (Points(0) - Points(2))).SafeNormal();
	
		DistStart = FPointPlaneDist( LineStart, Points( 0 ), PolyNormal );
		DistEnd = FPointPlaneDist( LineEnd, Points( 0 ), PolyNormal );
		if( (DistStart < 0 && DistEnd < 0) || (DistStart > 0 && DistEnd > 0 ) )
			return false;
	
		// Get the intersection of the line and the plane.
		IntersectionPoint = FLinePlaneIntersection( LineStart, LineEnd, Points( 0 ), PolyNormal );
		if( IntersectionPoint == LineStart || IntersectionPoint == LineEnd )
			return false;

    	if( FPointConvexPolygonIntersection( IntersectionPoint, Points, PolyNormal ) )
			return true;
	}
...
struct CORE_API FDimension
{
	FLOAT Width;
	FLOAT Height;
	FDimension() {}
	FDimension( FLOAT _Width, FLOAT _Height ): Width(Abs(_Width)), Height(Abs(_Height)) {}
	FDimension( const FPoint& P );	// Fix ARL: This might be a bad idea, since mistakes where you pass a Point instead of a Dimension won't be caught by the compiler.  But it makes doing things like PointA - PointB = DimensionC a lot simpler.
	FDimension operator=( INT Value ){ Width=Value; Height=Value; return *this; }

	// Unary operators.
	FDimension operator-() const { return FDimension(-Width,-Height); }

	// Binary math operators.
	inline FDimension operator*( FLOAT Scale         ) const { return FDimension( Width*Scale,   Height*Scale    ); }
	inline FDimension operator*( const FDimension& D ) const { return FDimension( Width*D.Width, Height*D.Height ); }
	inline FDimension operator/( FLOAT Scale         ) const { return FDimension( Width/Scale,   Height/Scale    ); }
	inline FDimension operator/( const FDimension& D ) const { return FDimension( Width/D.Width, Height/D.Height ); }
	inline FDimension operator+( const FDimension& D ) const { return FDimension( Width+D.Width, Height+D.Height ); }
	inline FDimension operator-( const FDimension& D ) const { return FDimension( Width-D.Width, Height-D.Height ); }
	...
	// Binary comparison operators.
	UBOOL operator==( const FDimension& D ) const { return Width==D.Width && Height==D.Height; }
	UBOOL operator!=( const FDimension& D ) const { return Width!=D.Width || Height!=D.Height; }

	// Assignment operators.
	inline FDimension operator*=( FLOAT Scale         ) { Width*=Scale;   Height*=Scale;    return *this; }
	inline FDimension operator*=( const FDimension& D ) { Width*=D.Width; Height*=D.Height; return *this; }
	inline FDimension operator/=( FLOAT Scale         ) { Width/=Scale;   Height/=Scale;    return *this; }
	inline FDimension operator+=( const FDimension& D ) { Width+=D.Width; Height+=D.Height; return *this; }
	inline FDimension operator-=( const FDimension& D ) { Width-=D.Width; Height-=D.Height; return *this; }

	// String conversions.
	FString String() const { return FString::Printf(TEXT("Dim(%f,%f)"),Width,Height); }

	// Blending.
	FDimension Blend( const FDimension& Other, FLOAT Pct ) const { return FDimension( appBlend(Width,Other.Width,Pct), appBlend(Height,Other.Height,Pct) ); }

	// Serialization.
	friend FArchive &operator<<( FArchive& Ar, FDimension& D ){ return Ar << D.Width << D.Height; }
};

struct CORE_API FPoint
{
	FLOAT X;
	FLOAT Y;
	FPoint() {}
	FPoint( FLOAT _X, FLOAT _Y ): X(_X), Y(_Y) {}
	FPoint operator=( INT Value ){ X=Value; Y=Value; return *this; }

	// Unary operators.
	FPoint operator-() const { return FPoint(-X,-Y); }

	// Binary math operators.
	inline FPoint operator*( FLOAT Scale         ) const { return FPoint( X*Scale,   Y*Scale    ); }
	inline FPoint operator*( const FPoint& P     ) const { return FPoint( X*P.X,     Y*P.Y      ); }
	inline FPoint operator/( FLOAT Scale         ) const { return FPoint( X/Scale,   Y/Scale    ); }
	inline FPoint operator+( const FPoint& P     ) const { return FPoint( X+P.X,     Y+P.Y      ); }
	inline FPoint operator+( const FDimension& D ) const { return FPoint( X+D.Width, Y+D.Height ); }
	inline FPoint operator-( const FPoint& P     ) const { return FPoint( X-P.X,     Y-P.Y      ); }
	inline FPoint operator-( const FDimension& D ) const { return FPoint( X-D.Width, Y-D.Height ); }

	// Binary comparison operators.
	UBOOL operator==( const FPoint& P ) const { return X==P.X && Y==P.Y; }
	UBOOL operator!=( const FPoint& P ) const { return X!=P.X || Y!=P.Y; }

	// Assignment operators.
	inline FPoint operator*=( FLOAT Scale         ) { X*=Scale;   Y*=Scale;    return *this; }
	inline FPoint operator*=( const FPoint& P     ) { X*=P.X;     Y*=P.Y;      return *this; }
	inline FPoint operator/=( FLOAT Scale         ) { X/=Scale;   Y/=Scale;    return *this; }
	inline FPoint operator+=( const FPoint& P     ) { X+=P.X;     Y+=P.Y;      return *this; }
	inline FPoint operator+=( const FDimension& D ) { X+=D.Width; Y+=D.Height; return *this; }
	inline FPoint operator-=( const FPoint& P     ) { X-=P.X;     Y-=P.Y;      return *this; }
	inline FPoint operator-=( const FDimension& D ) { X-=D.Width; Y-=D.Height; return *this; }

	// Comparison operators.
	UBOOL operator< ( const FRectangle& R ) const; // Returns true if this Point is inside of Rectangle R.
	UBOOL operator> ( const FRectangle& R ) const; // Returns true if this Point is outside of Rectangle R.
	UBOOL operator<=( const FRectangle& R ) const; // Returns true if this Point is inside or on the border of Rectangle R.
	UBOOL operator>=( const FRectangle& R ) const; // Returns true if this Point is outside or on the border of Rectangle R.
	UBOOL operator==( const FRectangle& R ) const; // Returns true if this Point is on the border of Rectangle R.
	UBOOL operator!=( const FRectangle& R ) const; // Returns true if this Point is not on the border of Rectangle R.
	UBOOL Contained ( const FRectangle& R ) const; // Returns true if this Point is inside or on the top/left border of Rectangle R (works better for adjacent rects).
	
	FLOAT Size() const { return appSqrt( X*X + Y*Y ); }
	...
	// String conversions.
	FString String() const { return FString::Printf(TEXT("Pt(%f,%f)"),X,Y); }

	// Blending.
	FPoint Blend( const FPoint& Other, FLOAT Pct ) const { return FPoint( appBlend(X,Other.X,Pct), appBlend(Y,Other.Y,Pct) ); }

	// Serialization.
	friend FArchive &operator<<( FArchive& Ar, FPoint& P ){ return Ar << P.X << P.Y; }
};

struct CORE_API FRectangle
{
	FLOAT X;
	FLOAT Y;
	FLOAT Width;
	FLOAT Height;
	FRectangle() {}
	FRectangle( FLOAT _X, FLOAT _Y, FLOAT _Width, FLOAT _Height )
	{
		X = _X; if(_Width<0.0f)  X+=_Width;
		Y = _Y; if(_Height<0.0f) Y+=_Height;
		Width = Abs(_Width);
		Height = Abs(_Height);
	}
	FRectangle operator=( INT Value ){ X=Value; Y=Value; Width=Value; Height=Value; return *this; }

	// Unary operators.
	FRectangle operator-() const { return FRectangle(-X,-Y,-Width,-Height); }

	// Binary math operators.
	inline FRectangle operator*( FLOAT Scale         ) const { return FRectangle( X*Scale, Y*Scale, Width*Scale,   Height*Scale    ); }
	inline FRectangle operator*( const FRectangle& R ) const { return FRectangle( X*R.X,   Y*R.Y,   Width*R.Width, Height*R.Height ); }
	inline FRectangle operator/( FLOAT Scale         ) const { return FRectangle( X/Scale, Y/Scale, Width/Scale,   Height/Scale    ); }
	inline FRectangle operator/( const FRectangle& R ) const { return FRectangle( X/R.X,   Y/R.Y,   Width/R.Width, Height/R.Height ); }
	inline FRectangle operator+( const FRectangle& R ) const { return FRectangle( X+R.X,   Y+R.Y,   Width+R.Width, Height+R.Height ); }
	inline FRectangle operator+( const FPoint& P     ) const { return FRectangle( X+P.X,   Y+P.Y,   Width,         Height          ); }
	inline FRectangle operator+( const FDimension& D ) const { return FRectangle( X,       Y,       Width+D.Width, Height+D.Height ); }
	inline FRectangle operator-( const FRectangle& R ) const { return FRectangle( X-R.X,   Y-R.Y,   Width-R.Width, Height-R.Height ); }
	inline FRectangle operator-( const FPoint& P     ) const { return FRectangle( X-P.X,   Y-P.Y,   Width,         Height          ); }
	inline FRectangle operator-( const FDimension& D ) const { return FRectangle( X,       Y,       Width-D.Width, Height-D.Height ); }

	// Binary comparison operators.
	UBOOL operator==( const FRectangle& R ) const { return X==R.X && Y==R.Y && Width==R.Width && Height==R.Height; }
	UBOOL operator!=( const FRectangle& R ) const { return X!=R.X || Y!=R.Y || Width!=R.Width || Height!=R.Height; }

	// Assignment operators.
	inline FRectangle operator*=( FLOAT Scale         ) { X*=Scale; Y*=Scale; Width*=Scale;   Height*=Scale;    return *this; }
	inline FRectangle operator*=( const FRectangle& R ) { X*=R.X;   Y*=R.Y;   Width*=R.Width; Height*=R.Height; return *this; }
	inline FRectangle operator/=( FLOAT Scale         ) { X/=Scale; Y/=Scale; Width/=Scale;   Height/=Scale;    return *this; }
	inline FRectangle operator+=( const FRectangle& R ) { X+=R.X;   Y+=R.Y;   Width+=R.Width; Height+=R.Height; return *this; }
	inline FRectangle operator+=( const FPoint& P     ) { X+=P.X;   Y+=P.Y;                                     return *this; }
	inline FRectangle operator+=( const FDimension& D ) {                     Width+=D.Width; Height+=D.Height; return *this; }
	inline FRectangle operator-=( const FRectangle& R ) { X-=R.X;   Y-=R.Y;   Width-=R.Width; Height-=R.Height; return *this; }
	inline FRectangle operator-=( const FPoint& P     ) { X-=P.X;   Y-=P.Y;                                     return *this; }
	inline FRectangle operator-=( const FDimension& D ) {                     Width-=D.Width; Height-=D.Height; return *this; }

	// Boolean operations.
	FRectangle Union( const FRectangle& R ) const
	{
		FLOAT MinX=Min(X,R.X);
		FLOAT MinY=Min(Y,R.Y);
		return FRectangle(MinX,MinY,Max(X+Width,R.X+R.Width)-MinX,Max(Y+Height,R.Y+R.Height)-MinY);
	}

	FRectangle Intersection( const FRectangle& R ) const
	{
		FPoint P(Max(X,R.X),Max(Y,R.Y));
		if( P<=*this && P<=R )
			return FRectangle(P.X,P.Y,Min(X+Width,R.X+R.Width)-P.X,Min(Y+Height,R.Y+R.Height)-P.Y);
		else
			return FRectangle(0,0,0,0);	// Fix ARL: Is there a better return value to indicate non-intersection?
	}

	// Conversions.
	FPoint     Point()     const { return FPoint(X,Y);              }
	FDimension Dimension() const { return FDimension(Width,Height); }

	// String conversions.
	FString String() const { return FString::Printf(TEXT("Rect(%f,%f,%f,%f)"),X,Y,Width,Height); }

	// Blending.
	FRectangle Blend( const FRectangle& Other, FLOAT Pct ) const { return FRectangle( appBlend(X,Other.X,Pct), appBlend(Y,Other.Y,Pct), appBlend(Width,Other.Width,Pct), appBlend(Height,Other.Height,Pct) ); }

	// Serialization.
	friend FArchive &operator<<( FArchive& Ar, FRectangle& R ){ return Ar << R.X << R.Y << R.Width << R.Height; }
};

struct CORE_API FAlignment
{
	BYTE XAxis;
	BYTE YAxis;
	FAlignment() {}
	FAlignment( BYTE _XAxis, BYTE _YAxis );
	FAlignment( EAlignment _XAxis, EAlignment _YAxis );
	FAlignment operator=( INT Value ){ XAxis=(BYTE)Value; YAxis=(BYTE)Value; return *this; }

	// String conversions.
	FString String() const { return FString::Printf(TEXT("Almt(%s,%s)"),GetEnumEx(Core.EAlignment,XAxis,5),GetEnumEx(Core.EAlignment,YAxis,5)); }

	// Serialization.
	friend FArchive &operator<<( FArchive& Ar, FAlignment& A ){ return Ar << A.XAxis << A.YAxis; }
};
