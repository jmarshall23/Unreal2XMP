...
//
#if 1 //NEW Fix: (mdf) -- report line number (will be 0 if script code not built with -lineinfo)
#define unguardexecSlow  unguardfSlow(( TEXT("(%s @ %s : %04X line %i)"), Stack.Object->GetFullName(), Stack.Node->GetFullName(), Stack.Code - &Stack.Node->Script(0), Stack.LineNum ))
#define unguardexec      unguardf    (( TEXT("(%s @ %s : %04X line %i)"), Stack.Object->GetFullName(), Stack.Node->GetFullName(), Stack.Code - &Stack.Node->Script(0), Stack.LineNum ))
#else
...
enum EFunctionFlags
{
	...
	FUNC_Multicast		= 0x00200000,	// Function is replicated to all clients for which the context actor is relevant
	FUNC_Interface		= 0x00400000,	// Funtion can be called from UI scripts.
...
{	
	...
	BYTE*		Locals;
	INT			LineNum;
	UBOOL		Native;	// Function using this frame was not called from UnrealScript.

	// Constructors.
	FFrame( UObject* InObject );
	FFrame( UObject* InObject, UStruct* InNode, INT CodeOffset, void* InLocals, UBOOL InNative=0 );
	...
	INT ReadWord();
	FName ReadName();
	void MaybeDumpTraceInfo( const UObject* Context, INT B ) const;
