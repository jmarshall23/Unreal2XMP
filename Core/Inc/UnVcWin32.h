...
extern CORE_API QWORD GStartCycles;	//NEW (arl)
CORE_API DOUBLE appSecondsSlow();
#if 1 //NEW (arl)
typedef union { struct { DWORD L,H; }; QWORD Q; } LARGEINT;
struct FCycles
{
	LARGEINT Value;
	FORCEINLINE FCycles()
	{
		...
		{
			rdtsc				// RDTSC  -  Pentium+ time stamp register to EDX:EAX.
			mov   [li.L],eax	// Save low value.
			mov   [li.H],edx	// Save high value.
		};
		Value=li;
	}
	FORCEINLINE FCycles(QWORD Q){ Value.Q=Q; }
	FORCEINLINE FCycles(DWORD L,DWORD H){ Value.L=L; Value.H=H; }
	FORCEINLINE FCycles operator-( const FCycles& C ) const
	{
		return FCycles(Value.Q - C.Value.Q);
	}
	FORCEINLINE QWORD operator-( const QWORD& Q ) const
	{
		return Value.Q - Q;
	}
...
{
	return ((DOUBLE)(FCycles() - GStartCycles)) * GSecondsPerCycle;
}
#else
inline DOUBLE appSeconds()
{
