...
public:
	...
	FColor Scale( FLOAT Scale, UBOOL AlphaOnly=0 ) const
	{
		FColor C = *this; if(Scale!=1.f){ if(!AlphaOnly){
		C.R = (BYTE)Clamp( ((FLOAT)C.R) * Scale, 0.0f, 255.0f );
		C.G = (BYTE)Clamp( ((FLOAT)C.G) * Scale, 0.0f, 255.0f );
		C.B = (BYTE)Clamp( ((FLOAT)C.B) * Scale, 0.0f, 255.0f );}
		C.A = (BYTE)Clamp( ((FLOAT)C.A) * Scale, 0.0f, 255.0f );}
		return C;
	}
	FColor Blend( const FColor& Other, FLOAT Pct ) const
	{
		return FColor( appBlend(R,Other.R,Pct), appBlend(G,Other.G,Pct), appBlend(B,Other.B,Pct), appBlend(A,Other.A,Pct) );
	}
	FString String() const
	{
		return FString::Printf(TEXT("Color(%d,%d,%d,%d)"),R,G,B,A);
	}
};
...
public:
	...
	virtual INT GetBaseIndex(){ return 0; }
};
...
// These are basically dummy classes used to keep the Cache information for 
// the UHardware shader so it can be in sync with FD3DPixel Shader and what-not.

class FShader : public FRenderResource
{
public:

	FShader(class UHardwareShader* InHardwareShader, INT InPass);
	virtual ~FShader();
