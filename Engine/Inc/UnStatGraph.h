...
public:
	...
	void Render(UViewport* viewport, FRenderInterface* RI);
 	...
 	UBOOL					bShowBarGraphs;
 	TArray<INT>				BarStats; // index to grab from GStat and graph.
 	struct BarStat
 	{
 		INT		statIndex;
 		FLOAT	desiredMax;
 	...
 	TMap<FString,BarStat>	BarStatMap; // hashtable keyed by stat name
 
 	class UMaterial* statBar1; // stat bar small is good image
 	class UMaterial* statBar2; // stat bar small is bad image
 	// end Demiurge
};
