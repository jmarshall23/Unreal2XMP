...
public:
	...
	// DrawArc - Buffers a wireframe arc for rendering.
	void DrawArc(FVector Base,FVector X,FVector Y,FLOAT StartAngle,FLOAT EndAngle,FColor Color,FLOAT Radius,INT NumSides);
...
public:
	...
	// staying lines
	TArray<FVector>	StayingLineStart;
	TArray<FVector>	StayingLineEnd;
	TArray<FColor>	StayingLineColor;
	...
	void AddStayingLine(FVector P1,FVector P2,FColor Color)
	{
		StayingLineStart.AddItem(P1);
		StayingLineEnd.AddItem(P2);
		StayingLineColor.AddItem(Color);
	}

	void ClearStayingLines()
	{
		StayingLineStart.Empty();
		StayingLineEnd.Empty();
		StayingLineColor.Empty();
	}
...
struct FTexturePoint
{
	FTexturePoint(){}
	FLOAT X,Y,U,V;
	FString String(){ return FString::Printf(TEXT("X:%f Y:%f U:%f V:%f"),X,Y,U,V); }
};

struct FQuadInfo
{
	FQuadInfo(){}
	FTexturePoint Pts[4];	//TL,TR,BR,BL
	FString String(){ return FString::Printf(TEXT("TL:%s TR:%s BR:%s BL:%s"),Pts[0].String(),Pts[1].String(),Pts[2].String(),Pts[3].String()); }
};

class ENGINE_API FImage : public Loki::SmallObject 
{
public:
	FImage(){}
	virtual ~FImage(){}

	FImage(FString InName,UMaterial* InMaterial,INT InX,INT InY,INT InW,INT InH,UBOOL InTile=0)
		: Name(InName),Material(InMaterial),X(InX),Y(InY),W(InW),H(InH),bTile(InTile){}
	friend FArchive &operator<<( FArchive& Ar, FImage& I )
		{ I.Serialize(Ar); return Ar; }
...
class /*UI_API*/ FUIRenderer
{
public:
	FUIRenderer(){}
	virtual ~FUIRenderer(){}

	virtual void		DrawQuad( UMaterial* Material, const FQuadInfo &Quad, FLOAT Z=0, DWORD PolyFlags=0, FColor Color=FColor(255,255,255,255) )=0;
	virtual void		DrawTile( FImage* Image, const FRectangle &Loc, FLOAT Z=0, DWORD PolyFlags=0, FColor Color=FColor(255,255,255,255) )=0;
	virtual INT			DrawString( const TCHAR* Text, FLOAT DrawX, FLOAT DrawY, class UFont* Font, FLOAT Z=0, DWORD PolyFlags=0, FColor Color=FColor(255,255,255,255), UBOOL bClip=false, UBOOL bHandleApersand=true )=0;
	virtual void		PushClip( const FRectangle &R )=0;
	virtual void		PopClip()=0;
	virtual void		PushScale( const FDimension &Scale )=0;
