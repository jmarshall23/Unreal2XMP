...
var	globalconfig int		Version;				// Version/Changelist number (displayed in beacon).

var	bool					bRestartLevel;			// Level should	be restarted when player dies
var	bool					bPauseable;				// Whether the game	is pauseable.
var	bool					bDisplayHud;			// Control display of various HUD interface	elements.
...
var config float			TeamDamage;				// Percentage of damage teammates inflict on each other.
var config int				PreMatchWait;			// Seconds to wait before allowing players to start the game

var string					DefaultPlayerClassName;

var	int						PauseKey;				// For locking SetPause.

// user	interface
var	  name					HUDType;				// HUD this game uses.
var	  name					ScoreboardType;			// Scoreboard this game uses.
...
{	
	return
		Level.ComputerName
	$	" ("        // TODO - DEB - Remove for release version
	$   Version     // TODO - DEB - Remove for release version
	$   ") "        // TODO - DEB - Remove for release version
	$	Left(Level.Title,24);
/*
	$	"\\t"
...
function RestartPlayer(	Controller aPlayer )	
{
	...
	// Append specific strings to base pawn class (team)
	NewPawnClassString = aPlayer.PawnClass.static.GetQualifiedClassName( aPlayer.GetTeam() );	
	aPlayer.PawnClass = class<Pawn>(DynamicLoadObject( NewPawnClassString, class'Class' ));
	...
	aPlayer.PlayerReplicationInfo.PawnClass = aPlayer.Pawn.Class;
...
function SetPawnClass( PlayerController PC, class<Pawn> NewPawnClass );
...
function AddDefaultWeapons( pawn PlayerPawn )
{
	...
	W = BaseMutator.GetDefaultWeapon();
	if( W!=None )
	...
	for( i=0; i<WeaponClasses.length; i++ )
	{
		...
		{
			newWeapon = Spawn(WeaponClasses[i],,,PlayerPawn.Location);
			if( newWeapon != None )
			{
				newWeapon.GiveTo(PlayerPawn);
				//newWeapon.BringUp();
				newWeapon.bCanThrow = false; //	don't allow default weapons to be thrown.
			}
		}
	}
}
...
function int ReduceDamage( int Damage, pawn	injured, pawn instigatedBy,	vector HitLocation,	out vector Momentum, class<DamageType> DamageType )
{
	...
	local float FloatDamage;
	local armor	FirstArmor;

	OriginalDamage = Damage;

	// Scale TeamDamage.
	if( TeamDamage > 0 && instigatedBy != None && instigatedBy != injured && injured.SameTeam( instigatedBy ) )
	{
		Damage *= TeamDamage;
		...
		if(Damage == 0)
		{
			FloatDamage = OriginalDamage * TeamDamage;
			if((FloatDamage > 0) && (FRand() < FloatDamage))
				Damage = 1;
		}
	}
	...
	else if	( (injured.Inventory !=	None) && (damage > 0) )	//then check if	carrying armor
	{
		...
		{
			Damage = FirstArmor.ArmorAbsorbDamage(Damage, DamageType, HitLocation, Momentum);
...
{
	BroadcastHandler.Broadcast(Sender,Msg,Type,MessageSound);
}

function BroadcastTeam(	Controller Sender, coerce string Msg, optional name	Type, optional sound MessageSound )
{
	BroadcastHandler.BroadcastTeam(Sender,Msg,Type,MessageSound);
}
...
{
	BroadcastHandler.BroadcastLocalizedTeam(Sender,Message,Switch,RelatedPRI_1,RelatedPRI_2,OptionalObject);
}
...
defaultproperties
{
	bDelayedStart=true // so we don't get warnings from entry level
	bWaitingToStartMatch=false
	bLoggingGame=false
	MaxPlayers=16
	GameDifficulty=+1.0
	...
	DeathMessageClass=class'DeathMessage'
