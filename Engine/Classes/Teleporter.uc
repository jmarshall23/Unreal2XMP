...
var() sound SourceSound,DestSound;
...
function PlayTeleportSounds( actor Source, actor Dest )
{
	if (SourceSound? && Source?)
		Source.PlaySound(SourceSound);
	if (DestSound? && Dest?)
		Dest.PlaySound(DestSound);
}
...
simulated function bool Accept( actor Incoming, Actor Source )
{
	...
	if ( Pawn(Incoming) != None )
	{
		...
		PlayTeleportSounds(Source, Self);
	}
	else
	{
