...
var		bool		bShowSelf;			// Pawn should show itself to other Pawns even if !bIsPlayer (!!mdf-tbd: see U2 optimizations, e.g. check team in native code, check health etc.)
...
var		bool		bShouldPossess;		//NEW (mdf) set to true if Pawn's using this controller should be possessed when spawned

//NEW (mdf) range tracking
var		float		RangeCounter;		//NEW (mdf) counter for checking for EnemyInRange events
var		int			LastRange;			//NEW (mdf) last range (1..Ranges.Num, 0 means not set
var		Pawn		LastRangeEnemy;		//NEW (mdf) last enemy used for range check
...
// Input buttons.
var input byte
	bRun, bSprint, bDuck, bJump, bFire, bAltFire;

var		rotator		AimRotation;		// rotation to used for aiming
...
replication
{
	...
	reliable if( RemoteRole==ROLE_AutonomousProxy )
		ClientGameEnded, ClientDying, ClientRevived, ClientSetRotation, ClientSetLocation,
...
//NEW (mdf)
event SeeEnemy( Pawn Seen );	// called when enemy seen (see ShowSelf for details)
event EnemyInvalid();			// called in first tick where current enemy set but invalid (Health < 0, bDeleteMe or bHidden)
/*OLD
...
function SetPawnClass(string inClass, string inCharacter)
{
    ...
    if ( pClass != None )
        PawnClass = pClass;
	if ( PlayerReplicationInfo != None )
		PlayerReplicationInfo.PawnClass = PawnClass;
}
...
State Dead
{
ignores SeeEnemy, HearNoise, KilledBy;
...
state GameEnded
{
ignores SeeEnemy, HearNoise, KilledBy, NotifyBump, HitWall, NotifyPhysicsVolumeChange, NotifyHeadVolumeChange, Falling, TakeDamage, ReceiveWarning;

	function BeginState()
	{
		if ( Pawn != None )
		{
//!!MERGE			if ( Pawn.Weapon != None )
//!!MERGE				Pawn.Weapon.HolderDied();
