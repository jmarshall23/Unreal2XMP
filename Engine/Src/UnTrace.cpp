...
UBOOL UModel::UseCylinderCollision( const AActor* Owner )
{
	guardSlow(UModel::UseCylinderCollision);
...
FBox UModel::GetCollisionBoundingBox( const AActor* Owner ) const
{
	...
	return Owner ? BoundingBox.TransformBy(Owner->LocalToWorld()) : BoundingBox;
	unguard;
}
...
struct FBoxCheckInfo
{
	...
	void SetupHulls( const FBspNode& Node )
	{
		...
		for( NumHulls=0; HullNodes[NumHulls]!=INDEX_NONE && NumHulls<ARRAY_COUNT(Hulls); NumHulls++ )
		{
			...
			if( Owner )
				Hull = Hull.TransformByUsingAdjointT( Matrix, MatrixTA );
...
{ \
	...
	{ \
		...
		if \
		(   !ClipTo( FPlane( 0.f, 0.f,-1.0,0.1-Box.Min.Z).TransformByUsingAdjointT(Matrix, TA), INDEX_NONE ) \
		||	!ClipTo( FPlane( 0.f, 0.f,+1.0,Box.Max.Z+0.1).TransformByUsingAdjointT(Matrix, TA), INDEX_NONE ) \
		||	!ClipTo( FPlane(-1.f, 0.f, 0.0,0.1-Box.Min.X).TransformByUsingAdjointT(Matrix, TA), INDEX_NONE ) \
		||	!ClipTo( FPlane(+1.f, 0.f, 0.0,Box.Max.X-0.1).TransformByUsingAdjointT(Matrix, TA), INDEX_NONE ) \
		||  !ClipTo( FPlane( 0.f,-1.f, 0.0,0.1-Box.Min.Y).TransformByUsingAdjointT(Matrix, TA), INDEX_NONE ) \
		||	!ClipTo( FPlane( 0.f,+1.f, 0.0,Box.Max.Y-0.1).TransformByUsingAdjointT(Matrix, TA), INDEX_NONE ) ) \
			goto NoBlock; \
	} \
	\
...
struct FBoxPointCheckInfo : public FBoxCheckInfo
{
	...
	UBOOL BoxPointCheck( INT iParent, INT iNode, UBOOL Outside )
	{
		...
		while( iNode != INDEX_NONE )
		{
			...
			FPlane Plane; if (Owner) Plane = Node.Plane.TransformByUsingAdjointT(Matrix, MatrixTA); else Plane = Node.Plane;
...
)
{
	...
	if( Nodes.Num() )
	{
		...
		else
		{
			...
			do
			{
				...
				IsFront = Node.Plane.TransformByUsingAdjointT(Matrix, MatrixTA).PlaneDot(Location) > 0.f;
	...
	GStats.DWORDStats(GEngineStats.STATS_BSP_CollisionCycles) += (appCycles() - StartCycles);
...
)
{
	...
	while( iNode != INDEX_NONE )
	{
		...
		FLOAT Dist1	= Matrix ? Node->Plane.TransformByUsingAdjointT(*Matrix, TA).PlaneDot(Start) : Node->Plane.PlaneDot(Start);
		FLOAT Dist2	= Matrix ? Node->Plane.TransformByUsingAdjointT(*Matrix, TA).PlaneDot(End)   : Node->Plane.PlaneDot(End);
...
struct FBoxLineCheckInfo : public FBoxCheckInfo
{
	...
	void BoxLineCheck( INT iParent, INT iNode, UBOOL IsFront, UBOOL Outside )
	{
		...
		while( iNode != INDEX_NONE )
		{
			...
			FPlane          Plane; 
			if (Owner) 
				Plane = Node.Plane.TransformByUsingAdjointT(Matrix, MatrixTA); 
			else 
				Plane = Node.Plane;
...
)
{
	...
	if( Nodes.Num() )
	{
		...
		else
		{
			...
			// Truncate by 10% clamped between 0.1 and 4 world units.
			if( Trace.DidHit )
			{
				Hit.Time      = Clamp( Hit.Time - Clamp(0.1f,0.1f/Trace.Dist, 4.f/Trace.Dist),0.f, 1.f );
